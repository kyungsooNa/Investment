...
최종 업데이트: 2026-08-21 (현재가 거래소(KRX/NXT/통합) 분리 반영. 이전: 2026-07-05 bootstrap 분해 진전 · EventShadowManager 분리 · 해외/테마 계층 · 브로커 계층 계측)

규모(실측): 프로덕션 406개 파일 / ~101,300 LoC, 테스트 단위 421개·통합 35개 파일.
...
### 체결 스트림의 거래소 구분 (2026-08-21, #871/#872)

체결 TR은 셋 다 페이로드 포맷이 같아 수신 측에서 출처를 구분할 수 없었다. 그 결과 통합 틱이
종목코드 단위 캐시를 갱신하고, KRX 요청이 그 캐시를 그대로 돌려받아 "KRX인데 통합 시세" 가 됐다.

- `H0UNCNT0`(통합, KRX+NXT) — 전략·주문 경로가 쓰는 기준 스트림. `SubscriptionPolicy`(40슬롯)가 관리한다.
- `H0STCNT0`(KRX) / `H0NXCNT0`(NXT) — 화면 표시 전용. SSE 연결이 살아있는 동안에만 구독한다.
- 파싱 시 `_exchange`(KRX/NXT/UN) 태그를 붙이고, `PriceStreamService`는 SSE 큐를 `(종목, 거래소)`
  단위로 나눈다. **거래소 지정 틱은 `_latest_prices`·`StockRepository`·관심종목 알림·수집 통계를
  모두 건너뛴다** — 통합 기준 캐시를 오염시키지 않기 위함이다.
- 현재가 REST는 `exchange`에 따라 `J`/`NX`/`UN`으로 조회하며, 통합(UN) 요청만 캐시를 사용하고
  KRX/NXT 요청은 `force_fresh`로 우회한다 (`view/web/routes/stock.py`).
- 웹 UI 기본 거래소는 통합(UN)이다.
- 미검증: 실계좌에서 통합 구독과 거래소 지정 구독을 동시에 유지할 수 있는지, 모의투자에서
  `H0NXCNT0` 틱이 수신되는지.
...
- 단위 테스트는 `tests/unit_test/services`(99개 파일)를 중심으로 서비스/전략/스케줄러/태스크 전반을 폭넓게 덮음
