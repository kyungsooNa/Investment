...
- (비문서 변경) 현재 브랜치가 `main`/`master` 등 기본 브랜치이면 `codex/<작업요약>` 브랜치를 먼저 생성한다. 이미 작업 브랜치가 있으면 새 브랜치를 만들지 않는다.
...
- 이 봇은 종종 merge-base 가 아니라 최신 `origin/main` 과 2-dot 으로 비교해,
  **분기 이후 `main` 이 추가한 것을 내 브랜치가 삭제한 것처럼** 지적한다.
  이런 지적을 받으면 GitHub 의 Files changed 로 실제 diff 를 확인하고,
  오탐이면 PR 에 한 번 정정 코멘트를 남긴 뒤 진행한다.
- 다만 `review` 가 **실제 코드 지적**을 냈다면 그것은 버그 리포트로 취급해 검증하고 고친다.
  실패 사유가 외부 API 오류인지 실제 지적인지는 실패 잡 로그로 판별한다.
