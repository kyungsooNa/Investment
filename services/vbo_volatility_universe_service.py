"""VBO short-term-volatility specific liquidity universe service.

GenericLiquidityUniverseService 와 동일한 거래대금/시총 baseline 위에
20일 annualized close-to-close 변동성 floor(기본 0.35) 를 적용한다.
VBO(Volatility Breakout) 전략은 (open + range × k) 돌파 신호 자체가
종목의 변동성/range 에 비례하므로 변동성 낮은 종목은 신호 빈도와 edge 가
모두 부족하다. prefilter 단계에서 제외해 호출량과 노이즈를 동시에 줄인다.

용도: P2-2 universe 적합성 비교 ablation 변형 (`universe_vbo_volatility`).
"""
...
class VboVolatilityUniverseService:
    """5일 평균 거래대금 + 시가총액 + 20일 변동성 floor 를 적용하는 universe."""
    ...
    def __init__(
        ...
        *,
        ...
        min_volatility_20d_annualized: float = 0.35,
        ...
        self.min_volatility_20d_annualized = float(min_volatility_20d_annualized)
    ...
    async def get_watchlist(
        ...
        for row in (resp.data or []):
            ...
            market_cap = cap_billion * 100_000_000
            if market_cap < self._min_cap:
                continue

            ohlcv_resp = await self._sqs.get_recent_daily_ohlcv(code, limit=21)
            ...
            closes = [_safe_int(r.get("close")) for r in rows]
            volatility = annualized_return_std(closes)
            if volatility is None or volatility < self.min_volatility_20d_annualized:
                continue
