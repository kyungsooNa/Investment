...
게이트 (모두 통과해야 dispatch):
  ...
  2. market_open (market_calendar.is_market_open_now() 또는 market_clock.is_market_operating_hours() 가 False 면 차단)
...
class StrategyEventRouter:
    ...
    async def _is_market_open(self) -> bool:
        """MarketCalendarService 우선, MarketClock 주입 시 시간대 판정으로 fallback."""
        if hasattr(self._mc, "is_market_open_now"):
            return bool(await _maybe_await(self._mc.is_market_open_now()))
        if hasattr(self._mc, "is_market_operating_hours"):
            return bool(await _maybe_await(self._mc.is_market_operating_hours()))
        self._logger.warning("[EventRouter] market clock does not expose market-open check")
        return False
