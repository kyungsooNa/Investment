...
### 0. 불량
1. [전략스케줄러] 전략에서 매수/매도 API를 실패하는 경우가 종종 있음. 1.) 실패하면 log는 남기고, web에 남는 history는 저장이 안되는게 맞을거같고 2.) 실패하는경우에는 retry를 해야할거같음. retry 하는 queue를 만들어서 재시도하는 logic 추가가 필요. (주문관련 api가 우선순위가 높아야 할거같고, 그다음 조회 api)
