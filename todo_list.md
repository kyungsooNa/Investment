...
6. **[조건부·저위험 상시]**
   ...
   - **판단 대기 (저비용)**: ~~M-2 커버리지 회귀 방지 가드~~ · ~~M-2 가드 사각지대(bootstrap)~~ · ~~S-1 시장별 시작/정지 교차 경로 테스트~~ (**2026-09-17 3건 모두 완료** — CI `--cov-fail-under=94` · `MODULE_BUDGETS` · `test_it_scheduler_market_cross_control.py`) · D-2 다이제스트 AI 한도 소비 확인(남음)
...
- [ ] **AI 한도 소비 경로 확인** — X-4 에서 `by_type` 실측으로 뉴스 소비량을 판정했던 것과 같은 방식으로, 다이제스트가 `ai_usage` 집계에 잡히는지와 일일 한도(daily 100 − disclosure_reserve 20)에 미치는 영향을 확인한다. 배치성 축이라 인터랙티브 풀을 잠식하면 공시·종목 분석이 밀린다.
