...
- 남은 것: PIT universe snapshot과 상폐 OHLCV 산출물을 `BacktestReplay`/universe provider에 연결해 동일 기간 baseline(current-only) vs PIT(including delisted) 백테스트를 자동 재실행하고, 성과 격차를 정식 리포트에 통합한다.
