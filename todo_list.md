...
15. [모의투자기록]현재가 강제 재조회 버튼 클릭시, 하나만 클릭해도 전체 종목에서 실패 표시가 사라짐(모의투자기록 뷰등.) 실제로 재 조회가 되는건지, 조회는 한개만 했는데 버튼이 다 사라진건지 확인 필요.
...
- 코스닥/코스피 지수 직접 조회
  ...
  * **[추가개선] 오닐 스퀴즈 ** 
  1. [전략공통] 오닐 스퀴즈 전략은 몇주에서 몇달까지 추세를 추종하는 스윙/포지션 트레이딩인데 현재 스케줄러는 무조건 당일 청산하게 되어있음. 개별 전략별 당일 청산여부를 Strategy Config로 분리해서 관리해야함. ==> 내가 적용했는데 리뷰해줘
  2. [전략공통] STATE_FILE 가 제대로 store/load 되고 있는건지 모르겠음. 검증 필요. 만약 장마감시간에 프로그램이 종료되있는 상태라면 이후 복귀했을때 어떻게할지도 정해야함.
  3. [오닐] 현재 워치리스트(거래대금 상위30)가 당일 1번만 build 되게 되어있는데, 더 자주(프로그램 시작시 최초 1회. 프로그램 시작이후 다음 시점에 업데이트가 안되었으면 업데이트 - 장 시작후 10분, 30분, 1시간, 1시간 30분 정도?) build 되어야 할 것으로 보임
  3. 1[오닐] 또한, 현재 워치 리스트가 거래대금 상위30개로 보게 되어있는데, 여기서는 유의미한 종목을 찾을지가 미지수야, 차라리 거래대금 상위를 구하는 방식처럼 (상승률 상위30/ 거래량 상위30/ 거래대금 상위30) (하락률은 볼필요 없을듯) 3가지 상위30개를 더한 90개 watch_list 후보인 _analyze_candidate 에 등록하는게 맞을거 같아. 여기서 조건을 충족하는 30 or 60개 선정.
  ...
  6. [오닐] 현재 _analyze_candidate에서 최소 조건만 충족하면 순서대로 watchlist에 들어가게 되어있는데, 먼저 확인해야하는 우선순위가 필요함. 5일 평균 거래대금 비율(전체 시가총액 대비 거대대금이 얼마나 큰지) 또는 이격도 순으로 우선순위 산정 필요.
  7. 
