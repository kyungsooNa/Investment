...
- 남은 것(연결층 3b~4단계): (3b) universe PIT 증강 — `get_watchlist(date)` 후보 풀에 `PointInTimeUniverseProvider.delisted_codes_as_of(date)`를 합류시켜 상폐 종목이 동일 liquidity/RS 필터를 거치게 한다(universe 선정 semantics 변경 — 설계 결정 필요, 아래 옵션). build 스크립트 full-records(기간 union) export 선행. (4) baseline(current-only) vs PIT(incl. delisted) 자동 재실행 + 성과 격차를 정식 리포트(`StrategyLogReportService`)에 통합. 실제 격차 숫자는 FDR 데이터 생성 후에만 산출(데이터-ops 선행).
