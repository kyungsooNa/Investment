...
최종 업데이트: 2026-05-26 (P1 1-2 validation/ablation gate 반영)
...
- API budget limiter는 구조와 endpoint별 coverage/load invariant가 있으나, 실제 KIS 유량 한도 재확인과 운영 기본값 보정이 남아 있다.
...
- [x] walk-forward 결과를 전략 승격 gate에 연결한다. 단순 리포트가 아니라 기간별 안정성 미달 시 real mode 신규 진입을 막는 정책을 검토한다.
  ...
  - 완료된 부분(2026-05-26): `BacktestWalkForwardRunner` summary에 `validation_metrics_by_strategy`를 추가했다. 각 전략별로 `train_net_pnl`/`tune_net_pnl`/`test_net_pnl`, `in_sample_net_pnl`(train+tune), `out_of_sample_net_pnl`(test), in/out trade count, out-of-sample positive segment ratio를 집계한다. `scripts/run_backtest.py --walk-forward --profitability-gate` 경로는 이 summary metric을 `evaluate_strategy_profitability_gate(..., validation_metrics_by_strategy=...)`로 전달한다. gate는 기존 core test journal 판정은 유지하면서 validation metric을 strategy metrics에 병합해 PBO proxy 입력으로 사용한다. 신규 단위 테스트로 PBO 실패가 `multiple_testing_bias_warning` 및 best strategy block reason으로 전파되는 경로를 고정했다.
- [x] purged/embargo split 또는 같은 종목·인접 기간 누수 방지용 validation contract를 정의한다.
  - 완료된 부분(2026-05-26): `BacktestWalkForwardSegment`에 `embargo_dates`를 추가하고, `BacktestWalkForwardRunner` summary에 `split_contract`를 노출한다. `build_walk_forward_split_contract()`는 train/tune/embargo/test 날짜 overlap(`phase_date_overlap`), 요구 embargo 길이 미달(`embargo_gap_below_required`), embargo 없이 in-sample/test에서 같은 종목 코드가 재등장하는 경고(`same_code_in_sample_and_test`)를 검출한다. 이 contract는 현재 리포트/검증용이며, split 생성 자체는 기존 `embargo_days` 로 계속 방지한다.
- [x] ablation 결과를 승격 기준에 반영한다. 필터 제거/완화 시 성과가 크게 무너지지 않거나, 특정 필터가 실제로 기여하는지 확인한다.
  - 완료된 부분(2026-05-26): `compute_ablation_gate_summary()`를 추가해 ablation variant가 baseline보다 `total_net_pnl` 기준으로 지정 비율 초과 outperform하면 `ablation_variant_outperforms_baseline` 을 산출한다. `StrategyProfitabilityGateConfig.ablation_max_variant_outperformance_pct`를 추가했고 paper/backtest 기본값은 `None`(report-only), real-mode overlay 기본값은 `10.0`으로 설정했다. `scripts/run_backtest.py`의 period backtest + `--ablation` + `--profitability-gate` 경로에서 `result.ablation`을 gate에 전달하므로, ablation 결과가 있는 경우 전략 승격 차단 reason으로 반영된다.
...
- [~] API budget limiter 운영 정책을 실전 한도 기준으로 보정한다.
  ...
  - 완료된 부분(2026-05-25): endpoint별 coverage matrix를 코드/문서/테스트로 고정했다. `core.retry_queue.client_with_retry_queue.API_BUDGET_COVERAGE_MATRIX`가 현재가, OHLCV, 체결강도, 계좌/잔고, 주문 제출, 주문 취소, 체결/미체결 대사, WebSocket connect/subscribe/unsubscribe, emergency sell 경로의 category/lane을 선언한다. `docs/api_budget_coverage_matrix.md`에 운영 문서화했고, `TestApiBudgetCoverageMatrix`가 matrix와 런타임 라우팅 상수 및 limiter 기본 category 일치를 검증한다.
  - 완료된 부분(2026-05-25): 장초반 synthetic 부하 invariant 테스트를 추가했다. `test_opening_burst_load_keeps_order_reconcile_and_emergency_lanes_available`가 `quotation_price`/normal `order_submit` lane 점유와 대기열이 있는 상태에서도 `quotation_conclusion`, `account_reconciliation`, emergency `order_submit`이 독립적으로 진입하고, 같은 category의 추가 quote/order만 차단되는지 검증한다.
  - 완료된 부분(2026-05-25): `ApiBudgetLimiter.snapshot()`에 `rate_wait_total`과 `rate_wait_seconds_total`을 추가해 429 이후 대응형 retry에만 의존하지 않고 사전 throttle 대기 발생 여부를 운영 상태 API에서 관측할 수 있게 했다.
...
최신 main 리뷰 기준으로 남은 즉시 착수 항목은 전략 승격 검증 방법론 강화 쪽에 집중됩니다. 실전 데이터/장중 샘플이 필요한 항목은 별도 대기열로 둡니다.

1. **즉시 코드/정책 작업**
   - 실제 KIS 계정별 REST/WebSocket 유량 한도 재확인 후 API budget 기본값 보정 (P2 2-2, 운영 확인 필요)
