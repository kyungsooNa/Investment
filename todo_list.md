...
### 1. 환경 (Environment)
* *[Dedicated]* CLI View는 이제 사용안하니 제거.
### 2. 성능 (Performance)
...
### 4. 성능
* **[현재가차트]** 많이 빨라졌지만 아직도 차트가 1초내외로 기다려야 나오는 문제가 있음 우선순위 낮게 수정 필요.
...
### 1. 기능 (Features)
* **[신규 기능]** 전략 스케줄러 기능의 개선 (by claude) notepad++ 돌파매매전략 참고
...
* 1. 진입 기준 (돌파의 정의)
어떤 가격을 넘었을 때 '돌파'라고 판단할 것인지 명확한 수식이나 기준이 필요합니다.

가격 데이터 (OHLCV): 시가, 고가, 저가, 종가, 거래량 데이터는 필수입니다.

기준 지표: * 전고점/전저점: 최근 20일, 50일, 52주 신고가 등 특정 기간의 최고가 갱신 여부 (예: 돈치안 채널).

변동성 지표: 볼린저 밴드 상단 돌파 또는 ATR(Average True Range)을 활용한 변동성 돌파.

이동평균선: 단기 이평선이 장기 이평선을 강하게 뚫고 올라가는 정배열 초기 시점.

2. 가짜 돌파(Whipsaw) 필터링 정보
돌파매매의 가장 큰 적은 저항선을 뚫는 척하다가 다시 하락하는 '휩소(가짜 돌파)'입니다. 이를 걸러낼 보조 데이터가 필요합니다.

거래량 (Volume): 저항선을 돌파할 때 평소보다 압도적으로 높은 거래량이 동반되어야 진짜 추세일 확률이 높습니다.

모멘텀 지표: MACD, RSI 등을 활용해 돌파 시점의 상승 에너지가 충분한지 교차 검증합니다.

시장 지수 추세: 개별 종목이 돌파하더라도 코스피나 S&P 500 등 전체 시장(Market Trend)이 하락장이면 실패 확률이 높으므로, 거시적 장세 정보도 결합해야 합니다.
3. 리스크 관리 및 자금 배분 (포지션 사이징)
돌파매매는 승률이 낮고 손익비가 높은 특징이 있습니다. 10번 중 3~4번만 성공해도 크게 먹는 구조이므로 철저한 자금 관리가 핵심입니다.

손절선 (Stop-Loss): 진입 가격 대비 -2% 하락 시, 혹은 돌파했던 저항선을 다시 깨고 내려올 때 등 기계적인 손절 기준이 필요합니다.

자금 투입 비율: 잦은 손절이 발생할 수 있으므로, 전체 포트폴리오의 5% 정도만 할당하여 공격적인 전략으로 운용하는 식으로 리스크를 격리하는 자본금 분배 룰이 설정되어야 합니다.

청산 전략 (Trailing Stop): 수익이 날 때 어디서 익절할 것인지, 고점 대비 일정 비율 하락하면 수익을 실현하는 트레일링 스탑 로직이 필요합니다.

* **[신규 기능]** 포켓 피봇(Pocket Pivot) - 
1. 포켓 피봇 (Pocket Pivot) 포착을 위한 데이터
저항선을 뚫기 전, 주가 조정 구간(베이스) 내부에서 기관의 숨은 매집을 포착하는 이 책의 가장 핵심적인 진입 기술입니다.

과거 10일간의 하락 거래량: 오늘 상승한 거래량이 과거 10일 중 주가가 하락했던 날들의 거래량 중 가장 큰 거래량보다 커야 합니다. 즉, 일별 종가 등락 여부와 거래량 배열을 10일 치씩 슬라이싱해서 비교할 데이터가 필요합니다.

이동평균선 수렴도: 주가가 10일선 또는 50일선 내부나 근처에서 지지를 받으며 고개를 드는 시점을 찾기 위한 이평선 이격도 데이터가 필요합니다.
2. 매수 가능한 갭상승 (Buyable Gap-Up, BGU) 데이터
어닝 서프라이즈나 강력한 호재로 인해 엄청난 거래량을 동반하며 갭상승할 때, 이를 새로운 추세의 시작으로 보고 과감히 진입하는 기법입니다.

시가 갭(Gap) 비율: 전일 종가 대비 당일 시가가 얼마나 높게 형성되었는지 계산하는 틱 또는 일봉 데이터.

상대 거래량 (Relative Volume): 당일 거래량이 50일 평균 일일 거래량의 최소 1.5배(150%) 이상 폭발했는지 확인하는 데이터. (장중이라면 남은 시간을 계산해 예상 거래량을 추산하는 로직이 필요합니다.)

당일 최저가 데이터: BGU 기법은 '갭상승 당일의 저가'를 절대적인 손절선으로 잡기 때문에 장중 저점 데이터 저장이 중요합니다.

3. 시장 방향성 및 펀더멘털 데이터 (오닐 이론의 근간)
모랄레스와 캐처 역시 잡주가 아닌 시장의 주도주에만 집중했습니다.

펀더멘털 API: 분기별 주당순이익(EPS)과 매출이 전년 동기 대비 최소 25% 이상 급증했는지 필터링할 수 있는 기업 실적 데이터가 선행되어야 합니다.

시장 추세 지표: 나스닥, S&P 500 등 전체 시장 지수가 상승장(Uptrend)인지 확인하는 지수 데이터. 전체 시장이 하락세라면 포켓 피봇의 성공 확률은 급감합니다.

4. 청산 및 방어 로직 (매도 규칙)
7주 보유 규칙 (7-Week Rule): 돌파 이후 주가가 50일선을 깨지 않고 7주 이상 버틴다면 대시세로 갈 확률이 높으므로 계속 보유한다는 시간적 개념의 데이터 추적이 필요합니다.

거래량을 동반한 이평선 이탈: 10일선이나 50일선을 하향 이탈할 때, 하락 당일의 거래량이 평균보다 높은지 판단하는 필터링 로직이 필요합니다.
