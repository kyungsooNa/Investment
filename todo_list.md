...
- [~] 현재 proxy 기반 multiple testing 방어를 자금 확대 전 formal 검증으로 확장할지 결정한다.
  ...
  - 후속(미구현): formal PBO CSCV의 **T×N 행렬 데이터 파이프라인**(parameter-stability/ablation sweep의 config별 per-period 수익률 시계열 캡처 → 행렬 조립 → gate/리포트 배선). 알고리즘은 위로 준비됨. walk-forward는 `run_backtest --walk-forward`로 이미 존재(embargo 포함), purged validation(러너 변경) / ablation 자동 리포트(현재 console 전용·persist 없음)는 별도 단위.
