...
14. [프로그램매매] 선택한 종목바꿀때 차트만 잘바뀌고 표는 제대로 반영이 안됨.
  ...
  + 저장할 수 있는양이 정해져있나? 다른페이지 갔다오면 10시까지의 데이터만 다시 그려지는 버그가 있음
15.1 [모의투자 기록] 특정 전략을 골르면 차트에도 해당 전략에 해당하는 line만 남도록 수정 필요.
16. [전략스케줄러] 전략 실행시, API 문제로 매수가 안되면, 매수실패라고 알림이 옆에 뜨는데 실제로는 매수못했으니까 구매했다는 리스트에서도 빠져야하는게 맞을듯.
...
* **[개선 필요]** **[현재가차트]** 많이 빨라졌지만 아직도 차트가 1초내외로 기다려야 나오는 문제가 있음 우선순위 낮게 수정 필요.
...
### 5. 테스트 (Tests)
* **[개선 필요]** 실행시간이 오래걸리는 (10초는 너무 김) TC 개선필요 - sleep이 들어가있음. ==> 2초이내로 단축됨.
