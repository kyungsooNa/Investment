...
최종 업데이트: 2026-08-18 (08-07 이후 머지분과 문서 상태 대조 — 완료된 후속은 정리하고, 실제 남은 실행 항목만 유지. 이전 갱신: 2026-08-07 07-24~08-07 머지분 대조)
...
- [~] **(신규 2026-08-07) ES/호가 커버리지 붕괴로 코퍼스 10거래일 연속 백테스트 불가 — 진단**: 07-27~08-07 전 거래일 `valid_for_backtest=false`. 사유는 매일 동일한 `execution_strength_db_coverage_below_threshold` + `orderbook_db_coverage_below_threshold`(임계 50%). **분봉은 100% 정상**(07-21 수정 유효, `empty_minute_reasons={}`) — 07-22/23만 통과했고 그 뒤 전부 실패.
  ...
  - [ ] **다음 착수**: 익일(첫 장중 가동) `execution_strength_missing_reasons` 실측으로 미구독 vs 무틱 비중 확인 → (a) 미구독 지배 시 캡처 후보 PRICE 구독 확대(슬롯 예산 검토 동반) (b) 무틱 지배 시 QC 커버리지 정의를 '구독 가능 종목 대비'로 재조정. **실측 전 임계값부터 낮추지 말 것** — 게이트가 실제 결함을 잡고 있던 07-17 사례 재연 위험.
...
- [ ] 반복 수정이 몰린 두 지점의 회귀 커버리지 점검 — **종목코드 정규화**(6자리 zero-pad)와 **재시작 복원 경로**. 같은 결함이 알림 종류를 옮겨가며 재발한 패턴이라, 알림별로 흩어진 테스트가 공통 계약을 잠그지 못하고 있을 가능성이 높다.
- [ ] 알림 종류별 발신 조건 · 중복 억제 · 재시작 복원 정책이 한 곳에 정리돼 있지 않다 — 새 알림을 추가할 때 같은 실수를 반복하게 된다.
