...
- [~] 현재 proxy 기반 multiple testing 방어를 자금 확대 전 formal 검증으로 확장할지 결정한다.
  ...
  - 후속(미구현): purged K-fold validation(러너 변경, walk-forward+embargo는 `--walk-forward`로 이미 존재). formal PBO를 profitability gate에 hard 배선(현재는 리포트/콘솔/JSON 노출까지)할지는 canary 성과 데이터 축적 후 정책 결정.
