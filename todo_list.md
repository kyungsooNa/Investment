...
- [~] PR-2.5: VBO shadow 운영 관찰 시작.
  ...
  - 수집 현황(2026-06-13 재검증): `20260609`~`20260612` 4파일 모두 여전히 `subscriptions_refreshed`(status)뿐 — entry(`event_shadow`)/exit(`event_shadow_exit`) 0건. 후보 구독은 정상(일 10~26종목). 같은 기간 polling VBO 매수신호 8건(6/9 4건/6/10 1건/6/11 0건/6/12 3건). 코드 추적 결과 `PriceStreamService.on_price_tick → event_router.on_price_tick → evaluate_single` 디스패치 배선은 완전·정상이고, VBO `shadow_eval_stats` 가 4일 내내 한 건도 안 찍힘(=`evaluate_single` 미실행). 즉 끊긴 곳은 router 상류의 **tick 수신**. 단, (a1) WS 프레임 미수신 vs (a2) DataQuality 게이트 전량 거절은 둘 다 watchdog `subscribed_no_tick` 으로 동일하게 나타나 코드만으로 구분 불가. tick-레벨 로그(`logs/streaming/`)는 2026-06-01 이후 끊겨 진단 불가.
  - 진단 계측 추가(2026-06-13): a1/a2 분기용 종목별 tick-ingest 카운터 도입. `PriceStreamService` 에 `received`(유효 payload 진입)/`quality_reject`(품질게이트 탈락)/`dispatched`(router 전달) 누적 카운터 + `tick_ingest_stats_snapshot(codes)`. `StrategyScheduler` 에 optional `price_stream_service` DI 추가 → `subscriptions_refreshed` details 에 후보별 `tick_ingest` 스냅샷 첨부(미주입 시 no-op). `strategy_factory` 배선. 다음 장에서 `received=0`(a1 프레임 미수신) vs `received>0 & quality_reject≈received & dispatched=0`(a2 게이트 거절)을 event_shadow jsonl 에서 즉시 구분. 테스트: `test_price_stream_service_tick_ingest_stats.py` 5종 + `test_strategy_scheduler_event_shadow.py` 2종. 단위 5942 / 통합 240 passed. (관측 전용, 실주문 경로 무변경) 별도 미해결: `logs/streaming/` 로거가 6/1 이후 파일 미생성 — 후속 조사 필요.
