...
- [ ] **[프로그램매매]** 실제 구독중인 내용과 web에서 보여지는 내용이 다름.
- [ ] **[프로그램매매]** 장 마감 이후에는 웹소켓 관련 기능들이 돌 필요가 없음.
...
- [ ] **[전략스케줄러]** 시그널 실행 이력에서 매도시 매수가 대비 수익률도 같이 표기
...
- [ ] **[성능 프로파일링]** `PerformanceManager` 고도화를 통해 API 호출 대기시간 및 연산 병목 구간 분석.
...
### 1. 웹 UI 및 알림 기능
- [ ] **[UI 동적 제어]** 웹 UI에서 전략별 **최대 보유 종목 수(`max_positions`)를 동적으로 조절**하고 즉시 반영할 수 있는 기능 추가.
- [ ] **[시스템 알림]** 시스템 이벤트 및 주문 체결 내역을 보여주는 확장형(Expandable) 알림창을 웹 우측 상단에 추가. - 주문체결 정보/전략내부 동작에 대한 이벤트 주입이 덜 되어있음.
- [ ] **[외부 메신저]** 알림 발생 시 비동기로 Slack, Telegram 등으로 메시지를 발송하는 `NotificationService` 연동.
- [ ] **[투자결과]** 현재, 수익률만 표기하고 있지만 다음과 같은 지표 추가.
  1. 프로핏 팩터(Profit Factor = 총 수익금 ÷ 총 손실금) PF가 1.0 이하면 돈을 잃는 시스템이고, **1.5 이상이면 훌륭한 시스템, 2.0을 넘어가면 시장의 돈을 쓸어 담는 성배(Holy Grail)**로 평가합니다. => 1.5 이상 bold, 2.0 이상 더 진한 bold

  2. 1회 매매당 기대수익 (Expectancy)
  공식: (승률 × 평균 수익금) - (패배율 × 평균 손실금)
  예: 승률 40%로 평균 10%를 먹고, 60% 확률로 3%를 잃는다면? ➔ (0.4 × 10) - (0.6 × 3) = 4 - 1.8 = +2.2% => +가 나오는지(Bold).
