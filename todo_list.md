...
최종 업데이트: 2026-08-07 (07-24~08-07 머지분 ~85커밋 대조 — 완료분은 원칙대로 등재하지 않고, 그중 **남은 과제로 승격할 신호만** M-5·M-6·M-7 로 신설. 이전 갱신: 2026-07-23 실제 코드/DB 대조 정리)
...
- [~] **AI API 운영 하드닝**: **일일 호출량 차단 완료(2026-07-19)** — SQLite로 재시작 후에도 사용량을 유지하고 provider 기준 태평양 날짜에 초기화한다. 기본 100회 중 20회를 공시 요약에 예약해 종목·랭킹 분석은 80회에서 HTTP 429로 차단하며, 공시는 총 한도까지 계속 시도한다. `daily_request_limit: 0`이면 로컬 차단을 끌 수 있고 랭킹 UI에도 구체적인 한도 메시지를 표시한다. 429/일시적 5xx 재시도 정책은 완료(#694, 지수 백오프 2회). **잔여 4건 완료 (2026-08-07)** — 일일 한도는 총량만 막고 순간 폭주·입력 팽창은 막지 못한다는 갭을 `AiClient` 단일 초크포인트에서 해소: ① 서버 단위 동시 호출 상한(`max_concurrent_requests`, 기본 2) ② 연속 요청 최소 간격(`min_request_interval_sec`, 기본 1.0초) ③ 입력 컨텍스트 예산(`max_input_chars`, 기본 24,000자 — 초과 시 뒤를 자르고 경고) ④ 호출당 provider host·model·usage_type·elapsed_ms·프롬프트/완료/총 토큰·finish_reason 로그. 라이브러리 기본값은 전부 비활성이고 정책은 조립 지점(`ai_analysis` config)이 정한다.
...
- [ ] **Phase 5 안전/canary**: `get_overseas_balance`/`ccnl` reconcile(`OverseasReconcileService` scaffolding 존재), risk gate/kill switch/canary USD 확장, 실전 소액 canary, canary auto-fire 배선 + `live_enabled=True` 전환 — dry-run 검증 + canary 게이팅.
  ...
  - **USD 원장 Phase 2 — UI 성과 요약 (2026-08-17)**: `GET /api/overseas/trades` 를 소비하는 화면이 없어 `get_summary()` 가 노출되지 않던 것을 해소. 미국장 `보유·주문` 탭에 성과 요약(총/청산/승률/평균수익률) + 거래 목록을 붙였다. 원장은 로컬 DB 읽기라 탭 진입 시 자동 로드한다(잔고·미체결은 브로커 호출이라 버튼 유지). CANCELED lot 은 흐리게 + `취소` 라벨로 보유와 구분하고, 취소 건수는 "집계 제외"로 따로 밝힌다.
    - 남은 것: **대사 실행 UI 없음** — `POST /api/overseas/trades/reconcile` 은 API 로만 호출 가능하다(기본 dry-run, `apply=true` 로 보정). 수동 주문 빈도가 낮아 버튼을 먼저 만들지 않았다.
...
### M-6. 공개 배포 표면 안전 기준 [신규 — 2026-08-07]

- [ ] public demo mode · exact-origin CORS · 페이지 `Cache-Control: no-store` 가 도입됐으나(07-27~30), 아래 "운영 안전 기준 현황" 목록에 **공개 배포 항목이 없다**. 실전 계좌가 붙은 프로세스에 외부 접근 표면이 생긴 것이므로 기준에 편입한다. (`use_login=false` 에서 전 `/api` 가 401 로 막히던 버그도 이 표면에서 나왔다.)
