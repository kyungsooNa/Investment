...
- [~] PR-2.5: VBO shadow 운영 관찰 시작.
  ...
  - **다음 우선순위**: WebSocket price 채널 force-reconnect 미치유 루프(price_data_gap 단조 증가) 근본 원인 — 구독 종목 ≈55%가 종일 무틱이라 shadow parity 수집 자체가 불가. 라이브 실시간 데이터 품질 문제이므로 별도 우선 과제로 격상 검토.
...
- 남은 것(연결층 4단계 — run_backtest 배선 + 리포트): run_backtest에 opt-in 플래그(`--pit-universe <snapshot>`/`--delisted-ohlcv-dir <dir>`) 추가 → `StockQueryBacktestReplayService(delisted_ohlcv_store=...)` + universe를 `PointInTimeAugmentedUniverse`로 래핑(전략별 `item_factory` 공급). build 스크립트 full-records(기간 union) export 선행 필요. baseline(플래그 off) vs PIT(플래그 on) 자동 재실행 + 성과 격차를 정식 리포트(`StrategyLogReportService`)에 통합. **실제 격차 숫자는 FDR로 PIT 스냅샷·상폐 OHLCV 데이터 생성 후에만 산출**(데이터-ops 선행, 코드 배선과 분리). 배선은 run_backtest 핵심 경로 + 데이터 의존이라 데이터 생성 시점과 함께 진행 권장.
