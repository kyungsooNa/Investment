...
* **[개선 필요]** 성능 측정 담당 class 생성하여 각 함수별 동작별 시간 측정하고 오래걸리는 부분이 뭔지 분석하도록 하는 기능 제공 필요.
...
* **[신규 기능]** 체결량(체결대금: 체결량 * 체결가) 확인 API 추가. websocket
...
* v3
  1. 업종 소분류(테마) 주도 스코어링 (+20점) : URL: /uapi/domestic-stock/v1/quotations/search-stock-info, (실전 TR ID	CTPF1002R)
  std_idst_clsf_cd_name 표준산업분류코드명, idx_bztp_scls_cd_name 지수업종소분류코드명 참고
  2. 실시간 방아쇠: 체결강도(120%↑) & 고래 탐지(5,000만 원↑)
  내용: 종목이 20일 고점을 돌파하는 바로 그 '찰나'에, 실시간 체결강도가 120% 이상인지, 그리고 **건당 5,000만 원 이상의 시장가 대량 매수(고래)**가 꽂히고 있는지를 확인하고 매수하는 완벽한 가짜 돌파 필터입니다.

  V3로 미룬 이유: 현재 V2의 매수 스캔은 3분마다 한 번씩 도는 '폴링(Polling)' 방식입니다. 반면 체결강도와 고래 탐지는 웹소켓(H0STOUP0)이나 초당 REST 조회(inquire-ccnl)를 통한 1초 단위의 틱(Tick) 처리가 필요하므로, 스케줄러 아키텍처에 꽤 큰 부하와 구조적 변화를 요구합니다.

  V3 해결 과제: 돌파 임박(예: 고점 대비 -1% 이내) 종목만 별도로 추려서 웹소켓 스트림에 던져두고, 조건이 맞는 순간 이벤트(Event-driven) 방식으로 즉시 매수를 때리는 로직을 구현해야 합니다.
  3. 마켓 타이밍 지수 API 직접 연동
  내용: 현재 코스닥/코스피 시장의 우상향 여부를 판단하기 위해 훌륭한 아이디어인 **'ETF 프록시(KODEX 200, KODEX 코스닥150)'**를 사용 중입니다. 이를 실제 업종/지수 TR을 호출하는 방식으로 변경하는 것입니다.

- 스코어링 시스템 (RS 상대강도, 업종 소분류, 영업이익 /finance/financial-ratio)
  - [추가] 외국인/기관 3일 누적 순매수가 3일 누적 거래대금의 5%이상: +15점
...
- 코스닥/코스피 지수 직접 조회
  ...
  * **[추가개선] 오닐 스퀴즈 ** 
  ...
  6. [오닐,피봇공통] 주도주를 공통으로 사용하기 위한 코드공용화
  - 전략(Strategy) 클래스 안에 있던 종목 발굴 로직을 완전히 뜯어내어, 유니버스 관리를 전담하는 독립된 서비스 클래스를 만드는 방식입니다.

  구조: services/oneil_universe_service.py 생성

  역할: 1. generate_pool_a() (장 마감 후 수동 배치)
        2. get_watchlist() (장중 3중 그물망 호출 + 스코어링 + 캐싱)

  동작 방식: 스케줄러가 3분마다 전략 B와 전략 C를 실행할 때, 두 전략 모두 self._universe_service.get_watchlist()를 호출합니다. 서비스는 이미 갱신된 60개 리스트를 메모리에서 즉시 반환(캐싱)하므로, API 중복 호출이 0건이 됩니다.

  장점: 전략 B와 전략 C의 성과(수익률)를 스케줄러에서 독립적으로 추적할 수 있으면서도, API 리소스는 완벽하게 공유합니다.
