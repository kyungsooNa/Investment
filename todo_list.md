...
- **보류** — LiveStrategy 인터페이스 승격(`_position_state`/`_save_state`/`_bought_today` 정식 메서드화), 동기 DB/CSV 호출 to_thread 전환: 전략 7개 + 호출 경로 전반을 건드려 단독 PR 필요. 인터페이스 승격은 P3 3-4 lifecycle contract 재설계와 함께 다루는 것이 외과적.
  - 재검토 (2026-06-12): 두 우려 모두 당초 평가보다 **범위가 작다**. ① state 저장 블로킹은 이미 해소 — 전략 `_save_state()`는 이벤트 루프 안이면 `StrategyStateIO.schedule_save()` 백그라운드 atomic write 로 위임한다(P0 0-11). 남은 실제 블로킹 후보는 `VirtualTradeRepository.get_holds_by_strategy`의 호출당 `pd.read_sql_query`(DataFrame 생성) 하나 — `_run_strategy` 사이클당 2회+ 및 get_status 웹 폴링×전략 수로 호출된다. to_thread 일괄 전환 대신 **측정 → 해당 경로만 개선**(단기 캐시 또는 pandas 제거)이 외과적.
  - [x] 측정·적용 (2026-06-12, 브랜치 `perf/get-holds-latency`): 측정 결과 pandas 경로는 **행 수와 무관하게 호출당 ~9ms 고정**(200행/2,000행/10,000행 모두 median 8.7~9.3ms — pandas 오버헤드 지배), 동일 쿼리 raw cursor 는 0.06~0.48ms. get_status 1회(7전략)당 이벤트 루프 ~60ms 블로킹이 확인되어 `get_holds_by_strategy`만 raw sqlite 경로로 전환 — 전환 후 0.07~0.49ms (約 20~130배). 변환 계약(NULL→None, code 문자열 강제, market_regime JSON→dict, 깨진 JSON→None)은 전환 *전* 특성화 테스트 2종으로 잠근 뒤 동일 통과 확인. `get_holds`/`get_all_trades` 등 비핫패스는 pandas `_to_json_records` 유지(외과적 범위 한정). 단위 5900 / 통합 240 passed. ② 인터페이스 승격은 LiveStrategy 에 **getattr 위임 기본 구현**(position_state/persist_state/discard_bought_today/exclude_code_for_today/strategy_logger)을 추가하면 전략 7개 무변경으로 가능 — scheduler 호출부 7곳만 교체하는 소형 PR 로 P3 3-4 와 분리 가능. 부수 발견: LiveStrategy 의 `async save_state()` hook(L101)은 어떤 전략도 override 하지 않는 죽은 표면(`load_state`는 구현됨) — 승격 시 정리 대상.
