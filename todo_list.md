...
최종 업데이트: 2026-05-25 (최신 main 리뷰 재검토 반영)
...
- [x] `sell_all_stocks()`에 운영 목적별 청산 모드를 분리한다.
  ...
  - 후속(정책 합의 필요): 매도 청산 경로의 RiskGate 우회 정책, KillSwitch 청산 예외, API budget 우선순위(emergency 우선 처리), KillSwitchService auto-trigger 등 신규 호출처 추가는 별도 PR에서 진행한다.
...
- [x] 전략/서비스 전역 API budget limiter를 도입한다.
  ...
  - 남은 범위: 주문/WebSocket 메서드는 멱등성·상태 기반 동작 때문에 retry queue와 budget limiter 제외 목록을 유지한다. 따라서 "전역 API limiter 미도입"이 아니라 "주문/WebSocket budget 및 emergency 우선순위 정책은 별도 설계 필요"로 본다.
...
- [~] API budget limiter 운영 정책을 실전 한도 기준으로 보정한다.
  ...
  - 후속: order submit retry(`OrderExecutionService._ORDER_MAX_RETRIES = 3`)는 현재 코드 상수와 runbook을 일치시킨다. canary/full 단계별 retry 횟수 차등이 필요하면 config화 작업으로 별도 승격한다.
...
차단되지 않은 즉시 가능 작업이 거의 소진된 상태입니다. 남은 작업은 대부분 외부 데이터/운영 시간/조건부 트리거 대기입니다.

1. **운영 데이터 축적 대기 (코드 작업 없음)**
...
4. **코드 작업 가능하나 보류 결정된 항목 (필요 시 재승격)**
   - 주문/WebSocket API budget 및 emergency 청산 우선순위 정책화 (현재 주문/WebSocket은 retry queue/budget limiter 의도적 우회)
   - 주문 submit retry 횟수 config화 (현재 코드/문서는 `max_retries=3` 기준으로 일치)
