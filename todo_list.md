...
최종 업데이트: 2026-09-17 (08-30 이후 머지분 **#940~#979 40건 대조** — 그중 **38건이 미등재**였다. 핵심 판정 3건: ① **#964 가 08-18 의 ES/호가 커버리지 보장(캡처 후보 전체 PRICE 동반구독)을 되돌렸다** — 1-5 가 08-26 에 '해결' 로 닫은 커버리지 축이 구조적으로 재개방됐고 크리티컬 패스(코퍼스 valid 판정)에 직결한다. ② **M-5 알림 계약표가 낡았다** — 08-18 이후 알림 계열 5종(무역 트렌드·제주 pending·공시 점수 필터·장중 거래량 급증·YouTube 다이제스트)이 계약표에 없는 채로 늘었고, 그 계열에서 #950 중복 문구·#957 dedup 키 정규화가 실제로 재발했다. ③ **D-1 승격 조건 재판정: 여전히 승격 보류** (신규 엔드포인트 동반 수정이 다수). 함께: 국내 시간외 단일가 주문(#978)·앱 기동 견고성(#972/#976)·모의매매 화면 분리(#943~#946/#960) 신규 등재, Pool B 시총 하한 우회 **#979 로 해소**, 스케줄러 상태 가시성 재발 6건 계측. 5번 '착수 가능' 그룹을 위 ①②로 다시 채웠다. 앞선 갱신: 2026-08-30 (같은 날 후속: **M-9 계약 가드 테스트 신설 + 해외 잠금 사유 주석 정정 완료** — 5번 '착수 가능' 그룹을 비웠다. 앞선 갱신: 08-26 이후 머지분 #925~#938 대조 — 14건 중 **8건이 해외(미국장) 계열**: 장중 전략 6종 승격(#932)·미국장 마켓타이밍 게이트(#931)·실전 전환 P0 4건(#934)/P1 3건(#935)을 O-5/O-6 로 신규 등재하고 Phase 5 착수 조건을 갱신했다. 함께: **Phase 5 잠금 사유가 코드에서 stale** 인 것을 등재, **#936 이 '캡처 전용' 이던 랭킹 보충을 국내 VBO 트레이딩 후보 경로로 넘긴 정책 변경**을 Pool B 에 등재(시총 하한 우회 경로 포함), D-1 관세 계열 관찰 갱신(수렴 판정), M-11 홈 지수 카드 축 신규 등재, M-2 계측 갱신. 검증: 이 점검 시점 단위 9,808건·통합 299건 전부 통과. 이전: 2026-08-26 (M-9 계약 문서 `docs/quote_subscription_contracts.md` 신설 — 캐시 키/폴백 체인/슬롯 회계 3개 계약 + 추가 전 체크리스트. 잔여는 테스트 고정. 같은 날 앞선 갱신: 08-22 이후 머지분 #878~#923 대조 — **M-5 재발 신호 승격 판정: 조건 충족**(4일간 시세·구독 계열 9건, M-9 신설), 미등재 작업축 4건 신규 등재(S-1 스케줄러 시장 분리 · D-1 관세/무역 데이터 · D-2 YouTube 다이제스트 · M-10 해외 현재가 페이지), 0-2 강제청산 계열 재발 3건 기록, 커버리지 캠페인 완료 기록. 이전: 2026-08-22 08-18 이후 머지분 #853~#877 대조 — 미등재 작업축 2건 신규 등재(K-1 키움 브로커 연동 · O-4 해외 dry-run 3전략 확대), M-2 줄수 계측 갱신, M-5 재발 신호 추가. 이전: 2026-08-21 현재가 거래소 분리 후속 M-8 등재. 이전: 2026-08-18 08-07 이후 머지분과 문서 상태 대조 — 완료된 후속은 정리하고, 실제 남은 실행 항목만 유지. 이전 갱신: 2026-08-07 07-24~08-07 머지분 대조)
...
**2026-09-17 갱신 — 크리티컬 패스는 불변, 그러나 그 위에서 한 칸 되밀렸다.** 18일간 머지 40건 중 해외(미국장) 계열이 여전히 최대(#940/#952~#956/#959/#967~#971/#975 등 14건)이고 무역·알림 계열이 그 다음(#947~#951/#957/#966/#974/#977/#979 10건)이다. 엣지 입증의 전제인 **캡처 코퍼스**에서는 진전이 아니라 후퇴가 있었다 — #964 가 08-18 커버리지 보장을 되돌렸다(위 5-① · 1-5 항목). 코퍼스가 다시 invalid 로 떨어지면 1-8(백테스트 재실행)의 해제 조건 (a)가 같이 막히므로, 이번 점검의 최우선 확인 항목으로 둔다. 한편 08-30 에 '우선' 으로 올렸던 **Pool B 랭킹 보충의 시총 하한 우회는 #979 로 fail-closed 복원 완료**다(실계좌 주문 경로 항목이라 여기 기록한다).
...
5. **[착수 가능 — 외부 의존 없음]** (**2026-09-21 기준 등재 항목 완료.**)
   ...
   - **해외 실주문 잠금 사유 주석 정정 — 2026-08-30 완료**. 낡은 두 사유를 걷어내고 실제 사유(엣지 미입증)와 근거 리포트를 `overseas_bootstrap.py` docstring 에 명시했다. 동작 변경 없음. 상세는 Phase 5 항목.
6. **[조건부·저위험 상시]**
   ...
   - **관찰만 (조치 열지 않음)**: 0-2 강제청산 계열 재발 3건(08-26 이후 추가 0건 — 잦아드는 중) · D-1 관세/무역 계열(**2026-09-17 재판정: 승격 보류 유지** — 7건이 더 났으나 다수가 신규 엔드포인트·신규 파싱 동반이라 08-30 이 정한 기준("새 엔드포인트를 추가하지 않은 상태에서 기존 경로의 장애·스펙 수정 재발")에 걸리지 않는다. 단 **알림 하위 축 2건(#950/#957)은 M-5 로 귀속**해 위 5-②에서 다룬다. 근거는 D-1 항목)
   - **판단 대기 (저비용)**: ~~M-2 커버리지 회귀 방지 가드~~ · ~~M-2 가드 사각지대(bootstrap)~~ · ~~S-1 시장별 시작/정지 교차 경로 테스트~~ (**2026-09-17 3건 모두 완료** — CI `--cov-fail-under=94` · `MODULE_BUDGETS` · `test_it_scheduler_market_cross_control.py`) · ~~D-2 다이제스트 AI 한도 소비 확인~~ (**2026-09-21 완료** — `youtube` usage type 25일·365회, 인터랙티브 풀 최대 33/80)
7. **[실계좌 장중에서만 확인 가능 — 코드 완료]**
   - M-8 현재가 거래소 분리(KRX/NXT) 4건 실계좌 확인 (#871/#872, 2026-08-21 등재 이후 진전 없음)
8. **[착수 결정 대기 — 새 작업축]**
   - K-1 키움 REST 연동: Phase 4~7 **mock 범위 완료**(#889~#892, 2026-08-23) — 남은 것은 실 키 발급 후 스모크(1차 완료 판정)와 Phase 8~9 착수 범위 결정. 크리티컬 패스(엣지 입증) 아래이나, 2-4 무틱의 유일한 자체 우회로 후보이기도 하다.
   - D-1 관세/무역 데이터를 매매 신호로 연결할지 · D-2 다이제스트 채널/주기 정책 문서화 여부
...
- [x] **AI API 운영 하드닝 완료**: 일일 호출량 차단(2026-07-19), 429/일시적 5xx 재시도(#694), 순간 폭주·입력 팽창 방어(2026-08-07)를 완료했다. `AiClient` 단일 초크포인트에서 서버 단위 동시 호출 상한(`max_concurrent_requests`, 기본 2), 연속 요청 최소 간격(`min_request_interval_sec`, 기본 1.0초), 입력 컨텍스트 예산(`max_input_chars`, 기본 24,000자), provider host·model·usage_type·elapsed_ms·프롬프트/완료/총 토큰·finish_reason 로그를 처리한다. 라이브러리 기본값은 전부 비활성이고 정책은 조립 지점(`ai_analysis` config)이 정한다.
...
- [~] **(신규 2026-07-17) 분봉 캡처 결손 원인 계측·근본진단**: `get_day_intraday_minutes_list`가 특정 유동주(삼성전자우·HPSP·원익IPS 등 stable set)에 대해 첫 배치부터 empty/에러를 주는 원인을 진단.
  ...
  - [x] 변경 반영 후 다음 16:05 캡처에서 intraday coverage ≥80% 및 `empty_minute_reasons` 축소 확인 (2026-07-22 통과): 16:12 자동 캡처(후보 30종목=base 12+ranking 18)에서 intraday_coverage 100%, empty_minute_codes 0, empty_minute_reasons `{}`, `valid_for_backtest=true`. 분봉 11,730행(30종목×391행, 09:00~15:30). 잔여 경고 2건(execution_strength_db 66.7%·orderbook_db 63.3%, 폴백 10종목=랭킹 보충 대형주+정지주 EOD 스칼라 폴백)은 설계된 정상 동작.
...
- [~] **(신규 2026-08-07) ES/호가 커버리지 붕괴로 코퍼스 10거래일 연속 백테스트 불가 — 진단/수정 진행**: 07-27~08-07 전 거래일 `valid_for_backtest=false`. 사유는 매일 동일한 `execution_strength_db_coverage_below_threshold` + `orderbook_db_coverage_below_threshold`(임계 50%). **분봉은 100% 정상**(07-21 수정 유효, `empty_minute_reasons={}`) — 07-22/23만 통과했고 그 뒤 전부 실패.
  - 실측: 커버 종목 수가 26~31 → 13~17로 반토막. 로테이션 절단이 아니다 — 커버된 종목은 종일 안정 수신(08-07 기준 17종목 전부 234~381행). 결손의 다수는 랭킹 보충분(08-07 20종목 중 15종목 ES 0행), base도 14 중 6종목 0행.
  - 구조적 원인: ES·호가는 PRICE 체결 틱에 무임승차하는데 `ProgramCaptureSubscriptionTask`는 보통주 후보에 **PT만** 붙이고 PRICE는 우선주에만 붙인다([program_capture_subscription_task.py:180](task/background/intraday/program_capture_subscription_task.py:180)). 따라서 보통주 후보의 ES/호가 커버리지는 트레이딩·즐겨찾기·보유 등 **타 목적 PRICE 구독과의 우연한 교집합**에만 의존하며, 캡처 설계가 보장하는 값이 아니다. QC 임계 50%는 후보 34종목 중 17종목 이상이 타 목적으로 구독돼야 통과하는데 그 보장 장치가 없다.
...
- [blocked] 활성 전략 전체 walk-forward + Monte Carlo 재실행 + 민감도 표 — **2026-07-03 파일럿(VBO 6/15-30, PP 6/1-12) 결과 전 구간 0거래**였고, 현시점에는 PIT 후보/valid 캡처 코퍼스가 부족해 재실행 실익이 제한적이다.
  ...
  - 과거 원인 ②는 무효화: 6월 하순 양시장 MA 하락 → 마켓 타이밍 게이트가 스캔을 차단한다는 설명은 #766 이후 맞지 않는다. 현재 게이트는 스캔이 아니라 **매수 신호**를 막으며, 백테스트 쪽 parity는 #770/#844 후속으로 복원됐다. 따라서 전 구간 0거래 원인은 재확인 필요.
  - 해제 조건: (a) 1-5 캡처 코퍼스 중 `valid_for_backtest=true` 파일 축적(`replay_microstructure_*.json`의 `metadata.candidate_sources.base`가 당일 PIT 후보군 역할) 또는 (b) 시장 회복으로 Pool B 복원. ※ 2026-07-08 랭킹 보충으로 codes 폭 확대 — 단, 보충분은 필터 통과 '후보'가 아닌 유동성 상위 종목이므로 PIT 후보군으로 해석할 때 소스 구분에 유의. 2026-07-17부터 `metadata.candidate_sources`(base/ranking_supplement)가 파일에 기록되므로 PIT 후보군은 base만 사용(그 이전 파일은 로그 교차 참조 필요).
...
- [ ] **(신규 2026-08-26) 강제청산 계열 수정이 다시 3건 — 관찰 항목**: 위 "유출 종료" 판정(07-23) 이후에도 08-22~08-26 에 같은 계열에서 #899(보유수량으로 강제청산 매도 상한 제한) · #900(포지션 없는 강제청산 매도 실패 대사) · #901(대사된 강제청산을 모의매매 요약에서 제외)이 나왔다. 셋 다 **원 결함(절반 유출)의 재발이 아니라 그 주변 경계 조건**(보유수량 초과 매도 · 포지션 부재 · 요약 집계 오염)이라 0-2 를 다시 열지는 않는다. 다만 M-5→M-9 와 같은 계열 신호이므로, 다음 점검에서 강제청산 계열 수정이 더 쌓이면 청산 경로 계약도 M-9 방식으로 고정하는 항목으로 승격한다. **지금은 조치를 열지 않는다.**
...
### 0-3. KRX 시간외 단일가(애프터마켓) 주문 지원 [신규 등재 — 2026-09-17, #978]

미등재 상태로 머지됐고, **국내 실주문 경로를 넓히는 변경**이라 P0 에 둔다. `MarketClock.is_krx_after_market_hours()`(평일 16:00~20:00) 추가 → `MarketCalendarService.is_market_open_now(include_krx_after_market=True)` 로 장 운영 판정을 확장하고, 주문 타입·TR 분기(`korea_invest_trading_api.py`)와 주문 화면(`order.html`/`order.js`)에 시간외 단일가를 노출한다. `broker_order_submitter` · `order_submission_coordinator` · `order_execution_service` 가 함께 바뀌었다.

- [ ] **자동 전략 경로에 새 시간대가 새지 않는지 확인** — 확장은 `include_krx_after_market=True` 를 **명시한 호출자에게만** 적용되는 opt-in 구조다. 전략 스케줄러·강제청산(0-2 의 `FORCE_EXIT_TIERS`/`ORDER_CUTOFF`)·장마감 태스크가 `is_market_open_now()` 를 기본 인자로 부르는지 확인해, **16:00 이후에도 장이 열린 것으로 보이는 경로가 없는지** 고정한다. 강제청산 계열은 0-2 에서 이미 한 번 데인 축이다.
- [ ] **시간외 단일가의 체결 의미 차이 반영 여부 판단** — 단일가는 10분 주기 체결이라 지정가/시장가 의미와 슬리피지 가정이 정규장과 다르다. 저널·체결 대사(`fill_reconciliation_service`)와 R-6 비용 모델이 이 주문을 정규장과 같은 가정으로 집계하면 성과가 왜곡된다. 현재 수동 주문 전용이면 영향은 작지만, **자동 경로에 열기 전에** 판단할 것.
- ※ 현재 범위는 **수동 주문 화면** 이다(전략 자동 경로에 시간외 주문을 내는 코드는 없다).

주요 파일: `core/market_clock.py`, `services/market_calendar_service.py`, `services/order_execution_service.py`, `services/order_submission_coordinator.py`, `services/broker_order_submitter.py`, `brokers/korea_investment/korea_invest_trading_api.py`, `view/web/routes/order.py`
...
주요 파일: `scripts/analyze_overseas_dryrun.py`, `services/overseas_{vbo,pocket_pivot,buyable_gap_up,channel_breakout,rsi2,squeeze_breakout}_dryrun_service.py`, `services/overseas_dryrun_suite_service.py`, `task/background/after_market/overseas_dryrun_task.py`
...
- [x] **빈 감시목록 latch 로 전략이 하루 통째로 죽던 버그 (2026-09-03)**: `prepare_session` 이 감시목록이 0개여도 `_session_date` 를 고정해, 개장 직후 한 번의 실패가 그날 전 폴링 패스를 no-op 으로 만들었다. 재시도 경로가 없어 **전략이 켜져 있어도 아무것도 하지 않는** 상태가 됐다. VBO 는 당일 시가가 필요해(당일 봉 미생성 시 전 종목 제외) 특히 잘 걸린다. 빈 결과는 latch 하지 않고 폴링 패스 기준 10회까지 재구축한다(`EMPTY_SESSION_RETRY_PASSES`). 재시도 패스가 보유를 지우지 않도록 상태 복원·포지션 초기화는 거래일당 1회로 제한했다.
- [x] **장중 태스크가 왜 idle 인지 화면에서 구분되지 않던 문제 (2026-09-03)**: #929 가 dry-run 에서 고친 것과 같은 증상 — 폴링 루프는 패스 사이에 IDLE 로 돌아오므로 `상태 idle` 만으로는 **장 마감·휴장·워밍업·감시 0개**가 전부 똑같이 보였다. `OverseasIntradayTask.get_progress()` 가 `phase`/`phase_detail`/`watch_count`/`session_date` 를 노출하고 스케줄러 화면이 마지막 패스의 사유를 표시한다. (프론트가 `progress.watch_count` 를 읽는 코드는 이미 있었는데 태스크가 채워주지 않아 항상 폴백 문구가 나왔다.)
- [x] **`0/2 실행`·`정지` 가 정상 가동을 멈춘 것처럼 보이던 문제 (2026-09-06)**: 시장 태스크의 `running` 은 실행 순간(폴링 틱·티켓 처리 중)에만 참이라, 정상 가동 중인 태스크도 화면에서는 거의 항상 `대기` / `0/2 실행` / `정지` 로 보였다 — **가동 중 대기와 미기동이 구분되지 않았다.** 태스크가 `armed`(장중=루프 살아있음, dry-run=WorkerPool 핸들러 등록됨)를 보고하고, 화면은 `태스크 가동 중` / `2/2 가동 (실행 중 0)` / 카드 배지 `가동 중(대기)` vs `미기동` 으로 가른다. 주말은 `정규장 시간이 아님` 이 아니라 `주말 — 미국장 휴장` 으로 적고, 평일 장외는 현재 ET 시각을 함께 싣는다. dry-run 카드에도 트리거 시각·마지막 실행 이력을 표시한다.
- **(2026-09-17 등재) 위 세 개선의 머지 번호와 그 뒤 추가분**: 빈 세션 재시도·idle 사유 노출은 **#956**, armed/stopped 구분은 **#962**, 섹션 헤더 표시는 **#965**, 해외 스케줄러 상태 문구는 **#952**, 미국장 태스크 배선 진단은 **#975** 다. 이 축의 재발 계측은 S-1 에 모았다.
- **(2026-09-17 등재) #940 해외 VBO MACD 필터 옵션** — `config` opt-in 으로 진입 필터를 하나 더 건다(dry-run·장중 양쪽). **전략 규칙이 바뀌었으므로 표본의 비교 기준이 갈라진다** — O-4 의 전략별 판정 집계 시 MACD on/off 를 구분하지 않으면 같은 라벨 아래 다른 규칙의 표본이 섞인다.
- **(2026-09-17 등재) #967/#968 RSI2 일봉 상한** — RSI2 의 200MA 추세 필터가 KIS 해외 일봉 **단일 응답 상한**을 넘겨 입력이 잘리던 것을 #967 이 상한 안으로 맞추고, #968 이 `market_data_service` 에 페이지네이션을 넣어 상한 너머까지 되감게 했다. 즉 **그 이전 RSI2 표본은 추세 필터 입력이 달랐을 수 있다** — O-4 판정 시 기준일을 확인할 것.
- **(2026-09-17 등재) #970/#971 호출량 구조 변경** — 모의 서버 해외 시세 budget 을 조이고(#970), 후보 스코어링 유니버스를 호출자 간 재사용한다(#971). O-3 의 `top_n` 상향 비용 판단 입력이 **또 바뀌었다**(합집합 크기 + 스코어링 재사용). 상향을 재검토할 때 08-30 의 '합집합 크기 먼저 재보기' 에 이 두 변경을 함께 반영할 것.
...
- [~] **Phase 5 안전/canary**: `get_overseas_balance`/`ccnl` reconcile · risk gate/kill switch/canary USD 확장 · 실전 소액 canary · canary auto-fire 배선 + `live_enabled=True` 전환.
  ...
  - **USD 원장 Phase 3 — 자동 전략 기록 경로 (2026-09-02, #954 · 커버리지 보강 #959)**: 장중 전략이 실제로 돌아 진입·청산 알림과 would-be 저널이 남는데도 미국장 모의투자 화면은 항상 "기록 없음" 이었다. **원인은 원장에 쓰는 경로가 웹 수동주문(`view/web/routes/order.py::_record_overseas_trade`) 하나뿐이었던 것** — 자동 경로는 `OverseasOrderExecutionService` 가 저널·알림만 남기고 원장에 쓰지 않았고, 국내 원장은 `is_domestic_virtual_trade_code`(#943)로 6자리 숫자 코드만 받아 미국 티커를 구조적으로 배제한다. #946 은 이 사실을 화면 문구로 설명만 했다. 기록은 주문 경로의 초크포인트인 `OverseasOrderExecutionService`(VBO 독립 구현 + 베이스 상속 5종이 모두 경유)에 `trade_repository` 주입으로 붙였고, 접수 성공(rt_cd=0)만 남긴다. `source` 는 주문별 `signal["strategy"]` — 자동 인스턴스 하나를 전략 6종이 공유하므로 인스턴스의 `journal_strategy_name` 으로는 전략을 구분할 수 없다. 청산은 `log_sell(source=...)` 로 **같은 출처 lot 만** 닫는다(심볼만으로 닫으면 전략 청산이 같은 심볼의 수동 보유를 대신 청산한다). 수동 주문 서비스에는 주입하지 않는다 — 라우트가 이미 기록해 이중기록이 된다.
- [x] **(2026-08-30 등재·같은 날 정정 완료) 잠금 사유가 코드에서 stale 했다**: `view/web/bootstrap/overseas_bootstrap.py:_build_intraday_strategies` docstring 은 잠금 근거를 두 가지로 적는데 **둘 다 이제 사실이 아니다** — ① "해외 주문 TR 은 실전만 존재" 는 #606 에서 모의 쌍이 추가돼 2026-08-11 에 이 문서가 이미 정정한 내용이고, ② "Phase 5(canary/kill-switch/reconcile) 가 미완" 은 #934/#935 로 kill-switch·reconcile 이 배선되며 대부분 해소됐다. 실제 잠금 사유(**스윕·교차검증에서 음의 기댓값**)는 코드 어디에도 적혀 있지 않고 이 문서와 `reports/` 에만 있다.
  - 위험: 코드를 읽고 판단하는 사람에게는 **적힌 두 사유가 모두 충족된 것처럼 보이고**, 잠금 해제는 플래그 한 줄이었다.
  - **정정 완료 (2026-08-30)**: `_build_intraday_strategies` docstring 에서 낡은 두 사유를 걷어내고, 실제 사유(**엣지 미입증**)와 근거 리포트 2건, 그리고 플래그를 여는 조건이 "안전장치가 갖춰졌는가" 가 아니라 **"전략별 would-be 성과가 왕복비용(0.5%)을 넘겼는가"** 임을 명시했다. 동작 변경 없음.
  - 남은 같은 성격의 사례: Pool B 항목의 `_passes_validity_filter` docstring("fallback 시 fail-closed")이 #936 이후 실제 동작과 어긋난다. **이쪽은 주석만 고치면 문제를 덮는 꼴이라** 아래 시총 하한 확인 항목의 결정과 함께 처리한다.
...
- **계측 갱신 (2026-09-17 실측)**: `service_container.py` 943→**964줄 / import 54개** — 예산(980줄·56개)에 **줄수 16줄·import 2개 남았다**. 다음 서비스군 조립부터는 예산을 올리는 게 아니라 전용 bootstrap 으로 빼야 한다(가드 실패 메시지가 안내하는 경로). `overseas_bootstrap.py` 481→**515줄 / import 30개**(가드 대상 아님 — 아래 사각지대 항목) · `scheduler/strategy_scheduler.py` **2,526줄**(변동 없음) · `strategy_log_report_service.py` 2,243→**2,270줄**. 신규 관측: `services/trade_trend_service.py` **1,082→1,634줄**(D-1, 3주 만에 +51%) — 조립부가 아니라 가드 밖이고, 현재 god class 감시 목록에도 없다. 다음 증가 시 감시 대상에 넣을지 판단한다.
- **(2026-09-17) 운영 문서 동기화 2건**: #963 이 에이전트 지침(AGENTS/CLAUDE)의 **머지 게이트를 테스트 CI 단독**으로 명확히 했고(리뷰 봇은 게이트 아님), #961 이 `__Run.bat` 기동 전 갱신 절차를 손봤다. 코드 축은 아니지만 이 문서의 '게시 흐름' 과 짝이라 기록만 남긴다.
- [x] **(2026-08-30 등재 → 2026-09-17 완료) 가드의 사각지대 — 조립이 `overseas_bootstrap.py` 로 옮겨가 그대로 쌓이고 있었다**: 08-22 추출 당시 220줄이던 파일이 #931/#932/#934/#935 를 거치며 **481줄 / services·task import 30개**가 됐다(9일 만에 2.2배). `service_container.py` 의 예산이 지켜진 이유의 상당 부분이 "해외 조립이 저기로 갔기 때문" 이라 **가드가 측정하는 곳과 실제로 누적되는 곳이 어긋났다**. #888 이 고정한 것은 파일 하나이지 규약이 아니다.
  - **결정·완료 (2026-09-17): 가드를 넓혔다.** `MODULE_BUDGETS` 로 bootstrap 디렉터리 14개 모듈 전체에 (줄수, services·task import) 예산을 걸고, **목록에 없는 새 모듈이 생기면 실패**시킨다 — 사각지대가 다시 열리는 것을 구조로 막는다. 재분해(선택지 2)는 조립 순서를 건드려 위험이 더 크고, 예산을 넘겼을 때 판단해도 늦지 않다.
    - 등록 예산(2026-09-17 실측→예산): `overseas_bootstrap` 515→560줄 / 30→33 · `service_container` 964→980줄 / 54→56(기존 유지) · `strategy_factory` 252→300 / 2→4 · `scheduler_bootstrap` 206→250 / 0→2 · 나머지 9개 모듈도 동일 방식.
    - 검증: 미등록 모듈 추가 → 등록 가드 실패, `broker_bootstrap.py` 에 60줄 추가 → 예산 가드 실패. 둘 다 확인 후 원복.
...
- [x] **커버리지 회귀 방지 가드 — 완료 (2026-09-17)**: CI 테스트 스텝에 `--cov-fail-under=94` 를 걸었다.
  - **`.coveragerc` 가 아니라 CI 에 둔 이유**: `[report] fail_under` 는 **부분 실행 커버리지에도 걸린다** — `2.integration_test_with_coverage.bat`(통합만 실행)이 항상 실패하게 되고, 그런 가드는 무시하는 법을 가르친다. 하한은 전체 스위트를 도는 머지 게이트에만 의미가 있다. 예산 테스트(#888 방식)도 검토했으나 `coverage.xml` 이 없는 실행에서는 판정이 불가능해 취약하다.
  - **⚠ 숫자 함정 — 뱃지와 하한은 다른 값을 본다**: `.coveragerc` 가 `branch = True` 라 `--cov-fail-under` 는 **line+branch 합산값**과 비교한다. 2026-09-17 실측 **합산 94.69% / line-rate 96.21%**(뱃지는 line-rate). 뱃지 숫자를 보고 하한을 96 으로 올리면 통과할 수 없다. CI 주석에도 같은 경고를 남겼다.
  - 하한 94 는 현재값(94.69%)에 약 0.7%p 여유다. 내릴 일이 생기면 **왜 내리는지 여기 남길 것** — 조용히 내리면 이 가드는 계측이 아니라 장식이 된다.
...
### M-4. 히트맵(트리맵) [완료 — 미국·국내 장중 실시간 + 업종 축, 2026-08-14]

홈 화면 히트맵 2종 + 전용 페이지(`/heatmap`). 공용 렌더러는 `view/web/static/js/market_heatmap.js`.
...
- 국내: `/api/heatmap/domestic`. **장중에는 네이버 시총 페이지 실시간, 장외에는 daily_prices 종가** (#841).
  ...
  - **유니버스는 장중/장외가 같다** (#842). 네이버 시총 페이지는 ETF·ETN·우선주를 함께 싣는데
    `daily_prices` 는 담지 않아, 장이 열리면 KODEX 200·TIGER 미국S&P500 이 타일로 뜨고 중복 계상됐다.
    `get_snapshot_codes()` 로 저장소 유니버스를 넘겨 자른다.
  - **업종 블록** (#843): 네이버 업종(`sise_group.naver?type=upjong`) 79개를 `category_type='industry'` 로
    적재하고 라우트가 item 에 `sector` 를 채운다. 업종은 **배타적**(두 업종 이상 소속 0건)이라
    임의 배정 규칙이 필요 없다. top500 커버리지 99.6%. 수집 전이면 예전처럼 단일 그리드.
  - 수집은 `ThemeClassificationTask` 가 테마에 이어 같은 실행에서 한다(전역 수집시각 가드를 공유하므로).

기록 — 못 쓰는 소스와 그 이유 (2026-08-14 실측):
- `pykrx` 전반, FDR 비캐시 경로: `data.krx.co.kr/comm/bldAttendant/getJsonData.cmd` 를 치는데 KRX 가
  막아 세션 쿠키를 세워도 `LOGOUT` 을 돌려준다. **버전 업그레이드로 해결되지 않는다.**
- FDR `StockListing('KRX')`: GitHub 일별 CSV 캐시라 장중 값이 아니고 NXT 도 없다.
- NAVER **테마**를 대표 1개로 강제 배정하는 안은 채택하지 않는다(종목당 평균 2.7개 소속 → 화면 해석 왜곡).
  업종은 배타적이라 이 문제가 없어 채택했다.
...
07-28~08-06 에 알림 계열이 대폭 확장됐다(관심종목 가격 임계 · 시장 안전장치 사이드카/서킷브레이커 · 지수 임계 · 미국장 관심종목 등락 · report bot 분리). 기능 자체는 완료라 등재 대상이 아니지만, **같은 계열에서 수정이 반복된 것**은 남은 과제 신호다.
...
- [x] **(2026-08-22 등재 → 2026-08-26 승격 판정 완료) 인접 계열에서 같은 신호 재발 — 관찰 항목**: 아래 관찰 조건이 충족되어 **M-9 로 승격**했다. 관찰은 종료하고 실행 항목은 M-9 에서 관리한다. 원 관찰 기록은 근거로 남긴다: 08-18 문서화 이후 나흘(08-19~08-22)에 **시세 갱신·캐시 키·구독 슬롯 계열 수정 7건**이 다시 몰렸다 — #858 NXT 현재가가 종목코드 키 KRX 캐시로 제공 · #859 웹소켓 슬롯 계정이 KIS 등록분과 drift · #860 공통 페이지 지수 조회 hang · #863 즐겨찾기 자동 갱신·미국 히트맵 폴링 정지 · #864 히트맵 자동 갱신이 캐시를 우회 · #865 즐겨찾기 알림 부호 처리 · #867 즐겨찾기 가격 stale. M-5 가 정리한 것은 **알림** 계약이고, 이번 재발은 그 앞단인 **시세 캐시 키·갱신 주기·슬롯 회계** 계약이다(#871/#872 의 거래소 분리도 같은 축이며 M-8 미검증 항목이 남아 있다). 조치는 아직 열지 않는다 — 다음 문서 점검에서 이 계열 수정이 계속 쌓이면 캐시 키(코드 vs 코드+거래소)·갱신 주체·슬롯 회계 계약을 알림 계약과 같은 방식으로 문서화·테스트 고정하는 항목으로 승격한다.

- [x] **(신규 2026-09-17 등재 → 같은 날 완료) 계약표가 낡았다: 08-18 이후 늘어난 알림 5계열이 미등재**
  - 실측: `docs/notification_alert_contracts.md` 의 계약표는 **7행 그대로**(관심종목 국내 등락률·상한가, 미국장 등락률, 시장 안전장치/사이드카, 지수 임계, 마켓타이밍 일간, 운영자 알림). 그 뒤 추가·변경된 알림 계열은 **무역 트렌드 릴리스**(#947 제주 pending · #948/#949 MoM · #950 중복 첨부 · #951 Motie PDF 폴백 · #957 release dedup 키 · #977 확장) · **공시 점수 필터**(#966) · **장중 거래량 급증**(#974 신규 태스크 285줄 · #979 문구 정리) · **YouTube 다이제스트**(#977) · **즐겨찾기 알림 source 처리**(#955) 다섯이다.
  - 재발 근거(이게 승격이 아니라 '지금 하면 되는 일' 인 이유): #950 은 같은 첨부 문구가 두 번 실리는 것, #957 은 릴리스 dedup 키가 표기 차이로 갈라지는 것 — **둘 다 이 문서가 이미 '중복 억제' 와 '입력 키 정규화' 로 정리해 둔 실패 모드**다. 계약표에 없던 계열에서 그대로 반복됐다.
  - 가드 부재: M-9 는 문서 + 구조 가드(`test_quote_subscription_contract_guard.py`)로 고정했는데 **알림 쪽엔 가드가 없다**(실측 `grep -rn notification_alert_contracts tests/` → 0건). M-9 가드는 문서의 '관련 테스트' 표가 가리키는 파일 실재까지 검사해 **문서가 낡는 것 자체**를 막는데, 알림 문서에는 그 장치가 없어 한 달 만에 5계열이 빠졌다.
  - 할 일: ① 계약표에 위 5계열 추가(발신 조건 · dedup key · 재시작 복원 · 테스트 기준). ② 새 알림이 계약표에 없으면 실패하는 가드 추가 — 검출축 후보는 `services/telegram_notifier.py` 의 발신 메서드 집합과 `dedup_key` 사용처. ③ M-9 도입 때와 같이 **가드가 비어 있지 않음**을 합성 위반으로 함께 고정한다(allowlist 가드의 주 실패 모드가 '아무것도 안 잡는 가드' 이므로).
  - **완료 (2026-09-17)**: 계약표에 7행 추가(무역 트렌드 릴리스 · 제주 반도체 무역 · 제주 무역 pending · 장중 거래량 급증 · 공시 즉시 알림 · 공시 일일 요약 · YouTube 다이제스트)하고, 각 행에 **코드 진입점**을 백틱으로 명시했다. 공통 규칙에는 이번 점검에서 근거가 나온 두 가지를 추가했다 — ① **발송 성공 뒤에만 '보냄' 을 영속화한다**(#969: 감지와 발송 사이 1800초 창에서 죽어 거래일 20260908 해외 dry-run 티켓이 유실됐다) ② **점수 필터는 즉시 발송과 일일 요약에 같은 하한을 건다**(#966).
  - **가드 신설**: `tests/unit_test/services/test_notification_alert_contract_guard.py`. 검출축은 `telegram_notifier.py` 의 `send_*`(AST 수집 — 문자열 검색은 docstring 의 이름을 함께 잡는다). 새 발신 메서드는 **이벤트성 알림 / 정기 리포트** 중 하나로 분류되지 않으면 실패하고, 알림으로 분류하면 계약표에 진입점이 등장해야 통과한다. 계약표 빈 칸 금지와 '문서가 가리키는 테스트 파일 실재' 도 함께 건다.
  - **가드가 비어 있지 않음을 실제 코드로 확인했다**: ① 계약표에서 진입점 표기 1건 제거 → `test_contracted_alert_senders_appear_in_the_contract_table` 실패 ② `telegram_notifier.py` 에 미분류 `send_*` 1건 추가 → `test_every_telegram_sender_is_classified` 실패. 둘 다 확인 후 원복했다.
  - **남은 범위 한계(의도)**: `notification_service` 만 거치는 내부 알림은 detector 가 잡지 않는다(YouTube 다이제스트가 그 경우 — 계약표 행은 있으나 자동 검출 대상이 아니다). 내부 알림까지 넓히면 거의 모든 태스크가 걸려 신호가 죽는다. 가드 docstring 에 명시했다.
  - ※ 범위: 무역 계열의 **데이터 소스·엔드포인트** 축은 D-1 에 남기고, 여기서는 **알림 계약** 축만 다룬다.

### M-6. 공개 배포 표면 안전 기준 [완료 — 2026-08-18]

- [x] public demo mode · exact-origin CORS · 페이지 `Cache-Control: no-store` 를 "운영 안전 기준 현황"에 편입했다. 실전 계좌가 붙은 프로세스의 외부 접근 표면이므로 신규 공개 배포 변경은 이 기준을 함께 갱신한다. (`use_login=false` 에서 전 `/api` 가 401 로 막히던 버그도 이 표면에서 나왔다.)
...
### M-8. 현재가 거래소 분리 실계좌 검증 [신규 — 2026-08-21, #871/#872]

통합(H0UNCNT0) 틱이 KRX 표시값·종목코드 단위 캐시를 덮어쓰던 문제를 고치고(#871), KRX/NXT 탭에
거래소별 체결 스트림(H0STCNT0/H0NXCNT0)을 붙였다(#872). 코드 경로와 테스트로는 확인했으나 아래는
실계좌 장중에서만 확인 가능하다.

- [ ] 통합 구독과 거래소 지정 구독을 같은 종목에 동시에 유지할 수 있는지(KIS가 거절하지 않는지) 확인.
      거절되면 해당 탭은 갱신 없이 조회 시점 시세로 degrade한다(기존 동작).
- [ ] 모의투자에서 `H0NXCNT0` 틱이 실제로 수신되는지 확인. 미수신이면 NXT 탭 실시간은 실전 전용으로 표기.
- [ ] 장중 슬롯 사용량이 40 한도 안에 있는지 `/api/streaming/status`로 확인. 거래소 지정 구독은
      `SubscriptionPolicy` 슬롯 계산 밖이라 화면을 열어둔 동안 1슬롯을 추가로 쓴다.
- [ ] (엣지) SSE 종료 시점에 웹소켓이 끊겨 있으면 해지 요청이 실패해 재연결 복구 목록에 남을 수 있다.
      틱은 큐가 없어 무시되지만 슬롯 1개가 앱 재시작 전까지 유지된다. 실측되면 정리 경로 추가 검토.

### M-9. 시세 갱신 / 캐시 키 / 구독 슬롯 계약 고정 [M-5 에서 승격 — 2026-08-26]

M-5 가 관찰 항목으로 걸어둔 승격 조건("다음 문서 점검에서 이 계열 수정이 계속 쌓이면")이 **충족됐다**. 08-19~08-22 7건에 이어 08-22~08-26 나흘간 같은 계열에서 **9건**이 더 나왔다 — 빈도가 줄지 않았다.

실측 집계(#878~#923):

| 하위 축 | 머지 | 건수 |
|---|---|---|
| 수동 PT 구독 수명주기·슬롯 선점 | #906 구독 보존 · #907 슬롯 선점 · #908 REST 가격 갱신 · #910 가격 동반 구독 | 4 |
| 즐겨찾기 시세 갱신 주체·기준 | #897 폴백 신선도 검증 · #902 장외 등락률 기준 통일 | 2 |
| 구독 진단 | #909 즐겨찾기 알림 구독 진단 | 1 |
| 정적자산 캐시버스팅 | #913 v6 범프 · #915 버전 잠금 가드 | 2 |

판정 근거 — 이 계열이 반복되는 이유는 **계약이 코드에 흩어져 있고 문서·테스트로 고정된 적이 없기 때문**이다. M-5 의 알림 계열은 `docs/notification_alert_contracts.md` 로 고정한 뒤 재발이 잦아들었는데, 시세·구독 계열에는 대응 문서가 **없다**(`docs/` 확인 결과 캐시·구독·슬롯 계약 문서 부재).

- [x] **캐시버스팅 하위 축은 구조로 자체 해소됨 (#915)** — `view/web/asset_versions.json` 잠금 + `test_static_asset_versions.py` 가 템플릿 `?v=N` 과 파일 해시 불일치를 잡는다. 나머지 3개 하위 축에는 대응 가드가 없다.
- [x] **`docs/quote_subscription_contracts.md` 신설 (2026-08-26)** — 알림 계약 문서와 같은 형식으로 3개 계약을 고정했다. 추측이 아니라 코드 실상을 읽어 기술했다.
  - **계약 1 캐시 키** (#858/#871 축): 종목코드 단위 공유 캐시(`stock_price_repository`)는 **통합(H0UNCNT0) 시세 전용**이다. 거래소 지정 틱은 `price_stream_service._handle_tick` 이 공유 캐시를 건드리지 않고 `(code, exchange)` SSE 큐로만 fanout 하고, `stock_query_service.get_current_price` 는 `exchange != Exchange.KRX` 를 캐시 우회로 처리한다(`exchange_specific_bypass` 계측).
  - **계약 2 폴백 체인** (#897/#902 축): `rt_cd=="0"` 은 충분조건이 아니고 **값이 실제로 찼는지**로 다음 단계를 정한다(`_apply_price_rate` 3상태 — 없음/부분/완전). 단계 순서는 장 운영 여부에 종속된다(장외에 일봉을 먼저 두는 이유는 KIS 가 장전에 등락률 `0.00` 을 돌려주기 때문).
...
- [x] **계약을 테스트로 고정 완료 (2026-08-30)** — `tests/unit_test/services/test_quote_subscription_contract_guard.py`. `test_assembly_point_guard.py`(#888)·`test_static_asset_versions.py`(#915) 형식을 따라 **동작이 아니라 구조**를 건다. 기존 동작 테스트(`test_price_stream_service.py` 등)는 **현재 경로만** 덮어서, 새 서비스가 공유 캐시에 쓰거나 폴백 단계를 끼우거나 정책 밖에서 구독을 걸어도 전부 통과했다 — 계약이 반복해서 깨진 지점이 정확히 그 "새로 추가되는 경로" 였다.
  - 계약 1: 공유 현재가 캐시 쓰기 모듈 allowlist(현재 `market_data_service` · `price_stream_service` 2개) + 거래소 지정 틱 조기 반환이 공유 저장소 쓰기보다 **앞에 오는지**(AST 줄번호 비교 — 문자열 검색은 docstring 에 적힌 이름을 잡아 순서 판정이 뒤집힌다).
  - 계약 2: 장중/장외 폴백 단계 **순서 고정** + 모든 `_fill_from_*` 가 `_apply_price_rate` 를 거치는지 + `_apply_price_rate` 3상태(없음/부분/완전).
  - 계약 3: `MAX_WS_SLOTS == 40` · 우선순위 4단계 값 고정(새 우선순위 금지) · 정책 밖 직접 구독 모듈 allowlist · `set_external_reserved_slots` 호출처 고정 · `_calculate_used_slots` 가 내부 장부보다 브로커 원장을 먼저 보는지.
  - **가드가 비어 있지 않음을 함께 고정한다** — detector 를 합성 위반 소스로 검증하는 케이스를 넣었고, 도입 시 실제 코드를 일시 변형해(새 캐시 쓰기 1건 · 장외 순서 뒤집기 1건) 두 가드가 실제로 실패하는 것을 확인한 뒤 원복했다. allowlist 가드의 주된 실패 모드는 "아무것도 안 잡는 가드" 이기 때문이다.
  - 문서 자체가 낡는 것도 막는다: 계약 문서의 '관련 테스트' 표가 가리키는 파일이 실재하는지 검사한다.
  - ※ 실패는 "코드가 틀렸다" 가 아니라 **"계약을 확인하고 목록을 갱신하라"** 는 뜻이다 — 실패 메시지에 계약 요지와 문서 경로를 함께 싣는다.
- **(2026-09-17 갱신) 가드 도입 이후 이 계열 수정 3건 — 가드는 버텼고, 계약 밖에서 하나가 샜다**: #941(계약 가드 신설 머지 번호) · #958(`unmark` 를 스트림 타입 단위로 — 통합/거래소 지정 구독이 같은 종목에서 서로의 활성 표시를 지우던 문제) · #964(캡처 후보 PT/PRICE 동반 구독 회수). 세 건 모두 계약 가드를 통과했고 실제로 계약 위반도 아니다 — **다만 #964 는 '슬롯 회계' 계약 밖에 있는 것(캡처 코퍼스 커버리지)을 바꿨다**. 슬롯을 아끼는 변경이 다른 축의 보장을 걷어낸 사례이므로, 계약 3(슬롯 회계)에 "LOW 우선순위 구독을 줄이거나 회수할 때 그 구독에 얹혀 있는 캡처(ES·호가) 커버리지 영향을 함께 판단한다" 를 체크리스트 항목으로 추가할지 1-5 결론과 함께 정한다.
...
- **(2026-09-17 등재) #953 미국장 기능 커버리지 표기** — 해외 현재가 응답에 `feature_coverage`(지원/미지원 기능 목록)를 실어 화면이 "국내장에 있는 이 기능은 미국장에 없다" 를 밝힌다. ⚠ 이 목록은 `view/web/routes/stock.py` 의 **손으로 유지하는 상수**(`_OVERSEAS_STOCK_FEATURE_COVERAGE`)라, 기능이 늘거나 줄어도 자동으로 따라가지 않는다 — 이 문서가 반복해서 잡아온 "코드가 낡은 사실을 말한다" 패턴(Phase 5 잠금 사유 · Pool B docstring)과 같은 형태다. 미국장 기능이 추가될 때 이 상수를 함께 고치는 것을 잊으면 화면이 거짓말을 한다.
...
- **(2026-09-17 등재) 축이 아직 닫히지 않았다 — 2건 추가**: #942 카드에 차트 추가 · #973 지수 5분 주기 자동 갱신. 08-30 에 "남은 실행 항목 없음" 으로 적었으나 그 뒤로도 확장이 이어진다. #973 은 **갱신 주체·주기**를 프런트에 하나 더 얹는 변경이라 M-9 계약 2(폴백 체인)·계약 4(캐시버스팅)와 인접하다 — 지수 갱신이 서버 캐시를 우회하지 않는지는 #864(히트맵 자동 갱신이 캐시를 우회) 와 같은 실패 모드이므로, 다음 점검 때 이 축에서 수정이 또 나오면 M-9 계약표에 '지수/히트맵 자동 갱신 주기' 행을 추가할지 판단한다.
...
### M-12. 앱 기동·조립 순서 견고성 [신규 등재 — 2026-09-17, #972/#976]

기동 경로에서 2건이 났고 둘 다 **조립 순서/외부 의존** 축이라 함께 등재한다.
...
### M-13. 모의매매 화면 국내/해외 분리 + 당일 수익률 [신규 등재 — 2026-09-17, #943~#946/#960]

M-10/M-11 과 같은 성격의 **완료 축 기록**이다. 남은 실행 항목은 아래 1건.
...
### S-1. 전략 스케줄러 시장(국내/미국) 분리 [신규 등재 — 2026-08-26, #903/#904/#918/#919]

전략 스케줄러를 시장별로 갈랐다 — 미등재 상태로 4건이 머지돼 이번 점검에서 등재한다.
...
- [x] **(신규 2026-09-17 등재 → 같은 날 완료) 교차 경로·상태 표기 테스트 고정 — 같은 증상이 4번 반복됐다**: 08-30 이후 이 축에서 6건이 더 났다. #952 해외 스케줄러 태스크 상태 문구 · #956 빈 세션 재시도 + idle 사유 노출 · #962 armed 와 stopped 구분 · #965 섹션 헤더의 armed 표시 · #975 미국장 태스크 스케줄링 진단(배선) · #969 디스패치 날짜를 티켓 발행 뒤에만 영속화.
  - 성격 구분: **4건(#952/#956/#962/#965)이 "정상 가동인데 화면엔 정지·대기로 보인다" 는 동일 증상의 반복 수정**이고, 2건(#975/#969)은 배선·디스패치 상태 오염이다. #919(분리 직후 배선 결함) 이후 이 축이 잦아들지 않았다.
  - **완료 (2026-09-17)**: `tests/integration_test/test_it_scheduler_market_cross_control.py` 신설(10케이스). 기존 `test_it_scheduler_status_market_tasks.py` 는 **상태 조회만** 덮었고 정작 위험한 제어 경로가 비어 있었다 — 미국장 버튼이 국내 스케줄러를 멈추면 실주문 전략이 조용히 죽는다.
    - 고정한 계약 3개: ① 시장 인자가 대상 인스턴스를 고른다(시작·정지·전략 start/stop, `market` 생략 시 국내 — 하위 호환) ② **없는 시장은 폴백하지 않는다** — 미국장 스케줄러 부재 시 503 이고 국내 스케줄러를 건드리지 않는다(`_get_strategy_scheduler` 의 `market == "domestic"` 폴백이 비도메스틱까지 번지는 것이 이 계열의 전형적 실패다) ③ 상태 표기 — `armed` 를 알려주는 태스크는 IDLE 이어도 '가동 중 대기', 알려주지 않으면 `running` 폴백, STOPPED 는 둘 다 false.
    - **가드가 비어 있지 않음을 실제 코드로 확인했다**: ① `_get_strategy_scheduler` 의 비도메스틱 차단을 제거 → 폴백 테스트 2건 실패 ② `armed` 폴백을 `running` 고정으로 되돌림 → 표기 테스트 1건 실패. 둘 다 확인 후 원복했다.
    - **판단 결과 — 별도 계약 문서는 두지 않는다**: 표기 계약이 `running`/`armed`/`state` 세 값뿐이라 M-9·M-5 처럼 문서를 세우면 문서가 낡는 비용이 계약 자체보다 커진다. 같은 증상이 또 반복되면 그때 문서로 올린다.
- ※ 3-4 lifecycle 분해와 대상 파일(`scheduler/strategy_scheduler.py`)이 겹친다. 3-4 재승격 시 이 분리 구조를 전제로 범위를 잡을 것.

---

## D. 외부 데이터 소스 [신규 섹션 — 2026-08-26]
...
- [x] **재판정 (2026-09-17): 승격 보류 유지 — 단 알림 하위 축은 M-5 로 분리**. 08-30 이후 이 계열에서 7건이 더 났다: #947 제주 무역 pending 알림 · #948 월별 무역 MoM 파싱 · #949 국가 일평균 MoM 보정 · #950 관세 첨부 문구 중복 제거 · #951 **Motie PDF 폴백**(국가 일일 수출 변동의 업스트림 포맷 변경 대응) · #957 release dedup 키 정규화 · #977 알림·다이제스트 확장.
  - 08-30 이 정한 기준은 "**새 엔드포인트를 추가하지 않은 상태에서** 기존 경로의 장애·스펙 수정이 재발하면 승격" 이다. 7건 중 #948/#949/#947/#977 은 신규 파싱·신규 알림 동반이고, 순수 외부 대응은 #951 하나다 — **기준 미충족**이라 데이터 소스 축의 승격은 보류한다.
  - 다만 #950(중복 문구)·#957(dedup 키)은 데이터 소스 문제가 아니라 **알림 계약 문제**다. 이 둘은 M-5 계약표 공백에서 나온 것이므로 D-1 이 아니라 **M-5 에서 처리**한다(우선순위 5-②).
  - 다음 판정 기준은 그대로 두되, **#951 이후 Motie/관세청 포맷 변경 대응이 한 번 더 나오면** 그때는 신규 엔드포인트 동반 여부와 무관하게 승격 후보로 본다(외부 포맷이 계속 움직인다는 신호이므로).
- [ ] 전략·매매 경로와의 연결 여부 미정 — 현재는 조회 화면 전용이다. 매매 신호로 쓸 계획이 있는지 결정 대기.

### D-2. YouTube 다이제스트 [신규 등재 — 2026-08-26, #920/#921]

`services/youtube_digest_service.py` · `youtube_transcript_collector_service.py` · `gemini_youtube_video_analyzer_service.py` + `task/background/intraday/youtube_digest_task.py`(346줄).

- #920 부분 실패 알림 추가 · #921 이벤트 구동으로 전환(폴링 → 이벤트).
- [x] **AI 한도 소비 경로 확인 (2026-09-21)** — 일반 자막 요약은 `AiClient.complete(..., usage_type="youtube")`, Gemini 영상 폴백은 같은 `AiUsageLimiter.reserve("youtube")`를 사용하므로 `/api/ai/usage`의 `by_type`에 함께 잡힌다. `data/ai_usage.db` 실측(2026-08-09~09-19 Pacific 한도 기간)에서 `youtube`는 25일·365회, 일평균 14.6회·최대 21회였다. YouTube가 실행된 날 인터랙티브 사용량은 합계 535회 중 YouTube 365회(68.2%)였지만 일 최대가 33/80(41.3%, 잔여 47회), 전체 사용량 최대도 41/100이라 한도 차단이나 공시 예약분 침범은 없었다. 현재 채널 규모에서는 별도 YouTube 하위 한도를 두지 않고 공통 인터랙티브 풀을 유지한다. 채널·실행 주기를 늘릴 때는 일 최대치를 다시 확인한다.
- **(2026-09-17 등재) #977 이 다이제스트를 함께 건드렸다** — 무역 트렌드 알림 확장과 같은 PR 에서 `youtube_digest_service` 가 수정됐다. 알림 계약표 공백(M-5)에 다이제스트도 포함되므로 위 5-② 에서 함께 다룬다.
- [x] **수집 대상 채널·주기 운영정책 문서화 (2026-09-21)** — [`docs/youtube_digest_operations.md`](docs/youtube_digest_operations.md)에 채널 선정·제외 기준, 현재 5개 채널 기준선, 07:30 KST 일 1회·6시간 catch-up·24~96시간 lookback·채널당 3편 제한, 수동 실행/차단 재시도/Gemini 폴백 규칙을 고정했다. AI 예산은 `youtube` 일 30회 또는 전체 인터랙티브 일 60회를 운영 경보선으로 두고, 채널은 한 번에 하나씩 추가해 7회 예약 실행 후 유지 여부를 판단한다.
...
- [~] **Phase 6 주문 API 완료 (2026-08-23)**: `kiwoom_trading_api.py` 가 kt10000(매수)·kt10001(매도)·kt10003(취소)를 감싼다. 응답의 `ord_no` 는 소비처가 읽는 KIS 계약 키(`data["output"]["ODNO"]`)로 변환한다.
  - **주문 판정 규칙을 KIS 경로와 일부러 동일하게 맞췄다** — 브로커를 바꿨다고 같은 요청이 다른 주문이 되면 안 된다: `order_price > 0` 이면 지정가(`trde_tp=0`)·아니면 시장가(`trde_tp=3`), 지정가는 `adjust_price` 호가단위 보정, NXT 시장가는 거절. 시장가에는 `ord_uv` 를 아예 싣지 않는다.
...
- [x] **(2026-08-30 등재 → 2026-09-17 해소 확인) 랭킹 보충 후보의 시총 하한 우회 — #979 로 fail-closed 복원** — `_passes_validity_filter` 의 시총 검사는 `if market_cap > 0 and market_cap < min` 이라 **`market_cap == 0` 이면 통과**한다. 랭킹 응답에 `hts_avls`/`stck_avls`/`stck_llam` 이 없으면 `_parse_market_cap` 이 0 을 돌려주고, 그 다음 `intraday_rank` 조기 통과가 걸려 **시총 2,000억 하한이 한 번도 적용되지 않은 채** VBO 진입 평가까지 간다. 종전 fallback 경로는 `avg_5d_tv <= 0` fail-closed 가 backstop 이었는데, 조기 통과가 그 backstop 보다 앞에 있다.
  ...
  - **해소 (#979, 2026-09-16)**: `_passes_validity_filter` 가 `source == "intraday_rank"` 이고 `min_market_cap > 0` 인데 `market_cap <= 0` 이면 `market_cap_unknown` 으로 **거절**한다. 즉 시총 미상이 조기 통과로 하한을 건너뛰던 경로가 닫혔고, docstring 도 새 동작에 맞춰 갱신됐다("시총 미상은 fail-closed"). 08-30 이 제기한 "랭킹 API 가 시총 필드를 싣는지" 는 **싣지 않아도 안전한 쪽**으로 결론났다 — 다만 그만큼 랭킹 보충 후보가 통째로 탈락할 수 있으므로, 보충 경로의 실효 후보 수는 전략 로그로 확인할 것.
