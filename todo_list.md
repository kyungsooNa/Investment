...
최종 업데이트: 2026-05-23 (P3-4 strategy_id/config_hash 정리)
...
- [x] 총 노출 한도 외에 포트폴리오 집중도 리스크를 추가한다.
  ...
  - 4차 완료: `compute_portfolio_entry_pressure_summary()`를 추가해 profitability gate top-level report에 일자별 신규 BUY 진입 수를 포함한다. 하루 신규 진입이 threshold 이상이면 `portfolio_daily_entry_pressure_high` warning을 노출한다.
  - 5차 완료: `compute_portfolio_cooldown_summary()`를 추가해 profitability gate top-level report에 전략별 연속 손실 streak을 포함한다. 연속 손실이 threshold 이상이면 `portfolio_consecutive_loss_cooldown_candidate` warning을 노출한다.
...
- [~] scan cycle 단위 데이터 공유와 성능 계측을 강화한다.
  ...
  - 잔여 후속: order-to-fill latency, 동일 종목 current price/OHLCV memoization, in-memory aggregator + web API 노출 (dashboard 도입 시).
...
- [~] 활성 전략의 공통 scan/check_exits/state save/load 패턴을 base class 또는 helper로 추출할지 설계한다.
  ...
  - 남은 부분: scan/check_exits 공통 흐름을 base class/helper로 더 추출할지 별도 설계한다. `larry_williams_vbo`는 state 파일이 없어 마이그레이션 대상이 아니다.
- [~] `strategy_id`와 `display_name`을 분리한다.
  ...
  - 남은 부분: Phase 3 cosmetic — UI/log 의 `name` 표기를 `display_name` 으로 rename.
...
- [~] `TradeSignal` contract를 분석/운영 기준으로 확장한다.
  ...
  - 남은 부분: consumer 단계 — risk_gate/order_execution 가 `invalidation_price`/`stop_loss_price` 를 실제 정책 입력으로 사용. backtest replay 가 `required_data` 로 fixture 재현.
...
- [~] 설정 변경 통제를 도입한다.
  ...
  - 남은 부분: `virtual_trade_repository` DB schema 에 `config_hash` 컬럼 추가, config 변경 후 hash diff 를 확인하는 dry-run validation.
...
4. P3/P4 유지보수와 운영 품질
   ...
   - 남은 항목: 전략 scan/check_exits 공통 contract 설계, UI/log `display_name` rename, TradeSignal consumer 정책 연결
   - config hash 후속: virtual trade DB schema 컬럼 추가, config 변경 dry-run validation
