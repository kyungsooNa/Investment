...
6. **[조건부·저위험 상시]**
   ...
   - **판단 대기 (저비용)**: M-2 커버리지 회귀 방지 가드 · S-1 시장별 시작/정지 교차 경로 테스트(**2026-09-17: 근거 다시 강해짐 — 스케줄러 상태·배선 수정 6건 추가**, S-1 항목) · D-2 다이제스트 AI 한도 소비 확인
...
- [ ] **(신규 2026-09-17) 태스크 상태 표기 계약 고정 여부 판단 — 같은 증상이 4번 반복됐다**: 08-30 이후 이 축에서 6건이 더 났다. #952 해외 스케줄러 태스크 상태 문구 · #956 빈 세션 재시도 + idle 사유 노출 · #962 armed 와 stopped 구분 · #965 섹션 헤더의 armed 표시 · #975 미국장 태스크 스케줄링 진단(배선) · #969 디스패치 날짜를 티켓 발행 뒤에만 영속화.
  ...
  - 판단할 것: ① 위 교차 경로(시장별 시작/정지) 테스트 고정 — 근거가 처음 등재 때보다 더 강해졌다. ② 여기에 더해 **태스크 상태 표기 계약**(`running` / `armed` / `idle` / 미기동 · `phase`/`phase_detail`/`watch_count` 의 의미와 채우는 주체)을 M-9 식으로 문서·가드로 고정할지. 같은 증상이 네 번 반복된 이유는 표기 의미가 태스크·라우트·프런트 세 곳에 흩어져 있어서다(#962 의 "프런트는 `progress.watch_count` 를 읽는데 태스크가 채우지 않았다" 가 그 전형).
