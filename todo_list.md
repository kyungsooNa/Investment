...
- [ ] **(신규 2026-07-17) 분봉 캡처 결손 원인 계측·근본진단**: `get_day_intraday_minutes_list`가 특정 유동주(삼성전자우·HPSP·원익IPS 등 stable set)에 대해 첫 배치부터 empty/에러를 주는 원인을 `_capture_intraday`에서 종목별로 기록(rt_cd·msg / empty-first-batch / all-out-of-range / all-stale)해 `metadata.quality.empty_minute_reasons`에 노출 → 다음 캡처 런(16:05)에서 메커니즘 자가 진단. 근본원인 확정 후 수정(예: 실전 전용 API 배치/커서/우선주 처리·budget throttle). intraday 커버리지가 엣지 검증의 핵심 overlay이므로 우선순위 높음.
