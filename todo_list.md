...
- [~] 현재 proxy 기반 multiple testing 방어를 자금 확대 전 formal 검증으로 확장할지 결정한다.
  ...
  - 후속(미구현): formal PBO(CSCV — config별 T×N period 수익률 행렬 필요, 데이터 파이프라인 선행), walk-forward / purged validation(백테스트 러너 변경), ablation 자동 리포트. 수익률 skew/kurtosis metric은 현재 어디서도 계산하지 않아 DSR은 정규 가정 fallback 상태 — 모멘트 metric 추가는 별도 과제.
