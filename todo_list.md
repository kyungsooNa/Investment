...
### 3. Task 확장
- [ ] **[StrategyLogReportService, Task]** 장 마감 이후, 당일 발생한 전략 log file을 분석해서 각 전략별로 어떤 종목들이 매수에 다가갔는지, 매수한 이유가 무었인지, 매수 못한 이유는 무었인지에 대한 요약 정보를 report 해주는 background task 생성.
  1. 활성화되어있는 전략들을 찾는다.
  2. 그 전략들의 당일 latest 로그 파일들을 찾는다.
  3. 해당 로그파일들을 읽어서 각 전략별로 어떤 종목들이 매수에 다가갔는지, 매수한 이유가 무었인지, 매수 못한 이유는 무었인지에 대한 요약 정보를 report 해준다.
  # 구현 계획
        1. 기술적 타당성 검토
        현재 시스템의 로그 구조는 분석에 필요한 핵심 데이터를 이미 JSON 형태로 상세히 기록하고 있습니다.

        매수 종목 및 이유 포착 가능: 각 전략은 매수 신호 발생 시 buy_signal_generated 이벤트를 발생시키며, reason 필드에 매수 근거(예: "PP진입(20MA지지, 예상거래 150%...)")를 사람이 읽기 좋은 문장으로 남기고 있습니다.

        미매수 종목 및 사유 포착 가능: 전략 실행 중 조건에 부합하지 않아 탈락한 경우 breakout_rejected 또는 pp_rejected 이벤트를 기록하며, reason 필드에 구체적인 탈락 사유(예: low_program_net_buy, no_ma_proximity, poor_candle_quality)를 명시하고 있습니다.

        분석 대상 필터링: 로그에 timestamp와 전략 이름(name)이 포함되어 있어, 특정 날짜 및 전략별로 데이터를 그룹화하여 분석하기 매우 용이한 구조입니다.

        2. 리포트 생성 로직 제안
        Background Task는 다음과 같은 흐름으로 구현할 수 있습니다.

        로그 파일 스캔: 장 마감(예: 15:40) 이후 해당 날짜의 전략 로그 파일들(*SqueezeBreakout_1.log.json 등)을 읽어들입니다.

        데이터 분류:

        Success 그룹: event == "buy_signal_generated"인 항목을 추출하여 종목명과 매수 근거 요약.

        Near-Miss/Rejected 그룹: event == "*_rejected"인 항목들 중, 감시 대상(Watchlist)에는 들어왔으나 최종 관문을 통과하지 못한 종목들을 추출.

        데이터 정제: 동일 종목이 장중 여러 번 스캔될 수 있으므로, 종목별로 가장 마지막 상태나 가장 빈번한 탈락 사유를 기준으로 중복을 제거합니다.

        결과 출력: 생성된 요약을 Markdown 또는 이메일 포맷으로 변환하여 저장하거나 알림으로 전송합니다.

        3. 리포트 구성 예시 (가상)
        구현 시 다음과 같은 형태의 리포트 생성이 가능합니다.

        [2026-04-17 전략 실행 요약 보고서]

        1. 오닐스퀴즈돌파 전략

        매수 완료: 대덕전자 (OSB돌파|현재가>20일고가, 예상거래 120%, 강도 135%...)

        매수 실패:

        티엘비: 프로그램 순매수 부족 (low_program_net_buy)

        피에스케이: 윗꼬리가 너무 김 (poor_candle_quality)

        2. 오닐 포켓피봇 전략

        매수 실패:

        한선엔지니어링: 이평선 근접도 부족 (no_ma_proximity)

        4. 구현 시 고려사항
        로그 레벨: 현재 미매수 사유(rejected)는 DEBUG 레벨로 기록되고 있습니다. 리포트 생성을 위해서는 실행 시 로그 레벨이 최소 DEBUG 이상으로 설정되어 있어야 해당 데이터를 수집할 수 있습니다.

        파일 처리: StrategyScheduler 내에 MarketClock의 is_market_closed() 등을 활용하여 장 종료 후 1회 실행되는 DailyReportTask를 등록하여 자동화할 수 있습니다.
