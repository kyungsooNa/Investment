...
3. [프로그램매매] 정상동작 X
4. [모의투자결과View] 현재가 조회하는데 오래걸리는걸로 보이는데, 어떻게 할지 고민필요. (멀종종목조회 기능 추가 필요 /uapi/domestic-stock/v1/quotations/intstock-multprice, 실전 TR ID	FHKST11300006)
5. [오닐전략] POOLA 생성시 multi 종목 조회 기능 추가 필수.
...
- Pool A/B 분리 유니버스 + 동적 편입
    ...
    이 POOL은 생생해서 file로 저장해둬야곘네? 다음날 프로그램을 다시 시작해서 바로 활용할 수 있게?
    사실상 종목코드만 있으면될 것 같지만, 사용자도 확인하기 쉽게 주석으로라도 종목명과 선정이유 등을 같이 저장되었으면 좋겠어(성능을위해선 주석만하는게 좋을거같은데, 관리의 효율성을 위해선 어떻게해야할지?)
* v3
