...
- 남은 것(실제 격차 — 멀티데이 경로): 오버나잇 보유 전략에 대해 상폐 종말 손실(halt/정리매매)을 반영한 멀티데이 일봉 시뮬로 PnL 격차 정량화. 현재 일봉 근사 엔진은 종말 봉 미반영. `run_backtest --pit-universe`(#552)는 intraday 경로라 상폐 분봉 부재로 PnL 격차엔 부적합(노출 합류만 가능) — 멀티데이 일봉 시뮬은 별도 경량 harness 필요. `StrategyLogReportService` 자동 통합은 그 후.
