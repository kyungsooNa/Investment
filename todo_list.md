...
- 남은 것(연결층 2~4단계): (2) 상폐 OHLCV 로더 — 백필 산출물(`index.json`+종목별 CSV: date/open/high/low/close/volume/change)을 `get_recent_daily_ohlcv` KIS 행 스키마(`stck_bsop_date`/`stck_oprc`/`stck_hgpr`/`stck_lwpr`/`stck_clpr`/`acml_vol`)로 변환해 서빙. (3) 배선 — `StockQueryBacktestReplayService`가 primary sqs에 없는 상폐 코드는 로더로 fallback + universe 후보 풀을 `PointInTimeUniverseProvider`로 PIT-aware하게 증강(`GenericLiquidityUniverseService` 등 종목 리스트 seam). (4) baseline(current-only) vs PIT(incl. delisted) 자동 재실행 + 성과 격차를 정식 리포트(`StrategyLogReportService`, R-2/DSR 섹션 패턴)에 통합. 실제 격차 숫자는 FDR로 PIT 스냅샷·상폐 OHLCV 데이터 생성 후에만 산출됨(데이터-ops 선행).
