...
- [ ] **`docs/quote_subscription_contracts.md` 신설** — 알림 계약 문서와 같은 형식으로 3개 계약을 고정한다: ① **캐시 키 단위**(종목코드 vs 종목코드+거래소 — #858/#871 의 재발 원인) ② **시세 갱신 주체와 우선순위**(WebSocket 틱 / REST 폴링 / DB 스냅샷 폴백 체인과 각 단계의 신선도·완전성 판정 기준 — #897/#902 가 이 체인에서 났다) ③ **슬롯 회계**(`SubscriptionPolicy` 계산에 포함되는 구독과 그 밖의 구독, 선점·해지·복원 규칙 — #906/#907/#910 · M-8 의 거래소 지정 구독이 여기 밖이다).
- [ ] **신규 구독·시세 경로 체크리스트를 알림 계약과 같은 방식으로 붙인다** — 캐시 키 구성요소, 갱신 주체, 슬롯 계산 포함 여부, 재시작·재연결 복원, 폴백 단계별 판정 기준.
