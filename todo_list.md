...
- 남은 것(연결층 3~4단계): (3) 배선 — `StockQueryBacktestReplayService`가 primary sqs에 없는 상폐 코드는 `DelistedOhlcvStore`로 fallback + universe 후보 풀을 `PointInTimeUniverseProvider`로 PIT-aware하게 증강(`GenericLiquidityUniverseService` 등 종목 리스트 seam). build 스크립트 full-records(기간 union) export 선행 필요. (4) baseline(current-only) vs PIT(incl. delisted) 자동 재실행 + 성과 격차를 정식 리포트(`StrategyLogReportService`, R-2/DSR 섹션 패턴)에 통합. 실제 격차 숫자는 FDR로 PIT 스냅샷·상폐 OHLCV 데이터 생성 후에만 산출됨(데이터-ops 선행). 3단계는 백테스트 핵심 경로 변경이라 별도 신중 검토.
