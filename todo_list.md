...
- [ ] **AI API 운영 하드닝**: 서버 단위 동시 호출·재호출 제한, 429/일시적 5xx 재시도 정책, 입력 컨텍스트 예산, provider/model·지연·토큰 사용량 계측을 추가한다. 자동·반복 분석 확대 전 완료한다.
...
- [~] **(신규 2026-07-17) 분봉 캡처 결손 원인 계측·근본진단**: `get_day_intraday_minutes_list`가 특정 유동주(삼성전자우·HPSP·원익IPS 등 stable set)에 대해 첫 배치부터 empty/에러를 주는 원인을 진단.
  ...
  - [ ] 다음 캡처 런(16:05) 산출물의 `empty_minute_reasons` 분포 확인 → 지배 원인별 근본수정(예: 실전 전용 API 커서/페이지네이션·우선주 처리·budget throttle).
