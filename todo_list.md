...
- [~] 현재 proxy 기반 multiple testing 방어를 자금 확대 전 formal 검증으로 확장할지 결정한다.
  ...
  - 적용 완료(T×N 행렬 파이프라인 — formal PBO 실가동, 2026-06-16): `build_config_period_pnl_matrix(records_by_config)` 추가 — config별 journal records를 청산일(`signal_time` YYYYMMDD)별 net_pnl 합으로 묶어 기간 합집합 정렬 → CSCV 입력 형태(행=기간, 열=config) 행렬. `run_backtest --parameter-stability` 경로에 배선: 후보 = baseline + 모든 sweep point(`{dim}={value}`) → 행렬 조립 → `compute_pbo_cscv(n_splits=--pbo-cscv-splits, threshold=--max-pbo-cscv)` → `result.parameter_stability["pbo_cscv"]` 첨부 + 콘솔 `[PBO_CSCV]` 라인(`_format_pbo_cscv_console_lines`) + JSON payload. opt-in 플래그 `--pbo-cscv-splits`(기본 16)/`--max-pbo-cscv`(기본 None=보고만). 기간 부족 시 unavailable 우아하게 표기. 테스트: 행렬 빌더 3종 + 콘솔/플래그 7종(`test_run_backtest_pbo_cscv.py`). 단위 6217 / 통합 243 passed. → **formal PBO가 실제 parameter sweep 데이터로 가동됨**(이전엔 알고리즘만).
  - 후속(미구현): purged K-fold validation(러너 변경) / ablation 자동 리포트 persist(현재 console 전용). walk-forward는 `run_backtest --walk-forward`(embargo 포함)로 이미 존재. formal PBO를 profitability gate에 hard 배선(현재는 리포트/콘솔 노출까지)할지는 canary 성과 데이터 축적 후 정책 결정.
