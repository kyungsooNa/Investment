...
- 남은 것(연결층 4단계 — 실제 격차 산출): baseline(플래그 off) vs PIT(플래그 on) 두 번 `run_backtest --output json` 실행 → 기존 `scripts/report_survivorship_bias_impact.py`(PIT universe JSON + run 결과 입력, delisted-only/current-only SOLD net PnL 분리)로 격차 리포트. **실제 숫자는 FDR로 full-records 스냅샷(`--full-records`)·상폐 OHLCV 백필 데이터 생성 후에만 산출**(데이터-ops 선행). `StrategyLogReportService` 자동 재실행 통합은 별도 — 현재는 두 run 출력을 report 스크립트로 오프라인 비교하는 seam으로 충분.
