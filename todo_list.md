...
최종 업데이트: 2026-06-16 (P2 2-4 event-shadow 디스패치 갭 #551 수정 반영 — 잔여 블로커는 WS 피드 건강성 단독)
...
- P2 2-4: event-shadow 디스패치 갭(tick 수신 그룹) #551로 해소(StreamingService executor 오프로드가 async event_router 디스패치 차단하던 버그). 잔여 단독 블로커 = WS price 피드 무틱(구독 ≈55% 종일 no-tick) → shadow parity 5거래일 수집 자체가 막힘. event-driven live order는 별도 승인 전 No-Go.
...
- [~] 현재 proxy 기반 multiple testing 방어를 자금 확대 전 formal 검증으로 확장할지 결정한다.
  ...
  - 적용 완료(formal PBO CSCV 알고리즘, 2026-06-16): `multiple_testing_bias_service.compute_pbo_cscv(returns_matrix, n_splits=16, threshold=)` 추가 — Bailey/Borwein/López de Prado/Zhu(2014) Combinatorially Symmetric Cross-Validation. T×N(기간×config) 행렬을 받아 모든 C(S,S/2) 대칭 IS/OOS 분할에서 IS 최고 config의 OOS 상대순위 logit≤0 비율 = PBO. 블록 충분통계량으로 S=16(12,870조합)도 경량. 기존 `_compute_pbo_proxy`(스칼라 1분할 휴리스틱)는 불변 병행. `compute_multiple_testing_bias_summary`에 opt-in `returns_matrix`/`max_pbo_cscv_probability` 추가(기본 None → 동작 무변경, 미제공 시 `pbo_cscv.available=False reason=not_provided`), 임계 초과 시 warning `pbo_cscv_above_threshold`. 테스트: `test_multiple_testing_bias_service.py` 8종(dominant→PBO 0/noise→~0.5/config·period 부족 unavailable/odd splits 거부/임계 위반/summary 통합 2종). 단위 통과.
  - 적용 완료(T×N 행렬 파이프라인 — formal PBO 실가동, 2026-06-16): `build_config_period_pnl_matrix(records_by_config)` 추가 — config별 journal records를 청산일(`signal_time` YYYYMMDD)별 net_pnl 합으로 묶어 기간 합집합 정렬 → CSCV 입력 형태(행=기간, 열=config) 행렬. `run_backtest --parameter-stability` 경로에 배선: 후보 = baseline + 모든 sweep point(`{dim}={value}`) → 행렬 조립 → `compute_pbo_cscv(n_splits=--pbo-cscv-splits, threshold=--max-pbo-cscv)` → `result.parameter_stability["pbo_cscv"]` 첨부 + 콘솔 `[PBO_CSCV]` 라인(`_format_pbo_cscv_console_lines`) + JSON payload. opt-in 플래그 `--pbo-cscv-splits`(기본 16)/`--max-pbo-cscv`(기본 None=보고만). 기간 부족 시 unavailable 우아하게 표기. 테스트: 행렬 빌더 3종 + 콘솔/플래그 7종(`test_run_backtest_pbo_cscv.py`). → **formal PBO가 실제 parameter sweep 데이터로 가동됨**(이전엔 알고리즘만).
  - 적용 완료(ablation 경로 PBO 배선, 2026-06-16): `run_backtest --ablation`도 동일하게 후보(baseline + 모든 ablation variant)로 `build_config_period_pnl_matrix` → `compute_pbo_cscv` → `result.ablation["pbo_cscv"]` + 콘솔 `[PBO_CSCV]` + JSON payload. ablation variant는 feature-toggle 선택지라 "최적 variant 선택이 과최적화인가"를 묻는 정당한 multiple-testing 대상. `--output json`으로 ablation 결과가 이미 persist되므로 별도 ablation persist 작업은 불필요. 테스트 2종 추가(날짜 有→available/signal_time 無→unavailable 하위호환). 단위 6219 / 통합 243 passed.
  - 적용 완료(purged CSCV — embargo, 2026-06-16): `compute_pbo_cscv(embargo=N)` 추가 — IS/OOS 블록 경계 serial-correlation 누수 방지용 purge. 각 블록의 직전 블록이 반대편 partition이면 그 블록 head N개 관측치를 제외(블록별 full/trimmed 두 통계셋 precompute). `embargo>=block-1`이면 unavailable(`embargo_too_large`), 기본 0=무변경(하위호환). `run_backtest --pbo-cscv-embargo N`으로 parameter-stability·ablation 양쪽에 전달. 테스트 6종(echo/zero=default/적용 유효/too_large/dominant 불변/플래그). 단위 6225 / 통합 243 passed. → walk-forward(`--walk-forward`, embargo 포함)에 이어 CSCV 내 purging까지 확보. **P1 1-7 formal 방어 핵심(DSR+PBO+purge) 사실상 완료.**
  - 후속(미구현): formal PBO를 profitability gate에 hard 배선(현재는 리포트/콘솔/JSON 노출까지)할지는 canary 성과 데이터 축적 후 정책 결정. purged K-fold(별도 CV 러너)는 walk-forward+CSCV embargo로 실질 커버되어 우선순위 낮음.
...
- [~] PR-2.5: VBO shadow 운영 관찰 시작.
  ...
  - 수집 현황(2026-06-13 재검증): `20260609`~`20260612` 4파일 모두 여전히 `subscriptions_refreshed`(status)뿐 — entry(`event_shadow`)/exit(`event_shadow_exit`) 0건. 후보 구독은 정상(일 10~26종목). 같은 기간 polling VBO 매수신호 8건(6/9 4건/6/10 1건/6/11 0건/6/12 3건). 코드 추적 결과 `PriceStreamService.on_price_tick → event_router.on_price_tick → evaluate_single` 디스패치 배선은 완전·정상이고, VBO `shadow_eval_stats` 가 4일 내내 한 건도 안 찍힘(=`evaluate_single` 미실행). 즉 끊긴 곳은 router 상류의 **tick 수신**.
  - **streaming 로그 분석(2026-06-13, 정정)**: `logs/streaming/`는 끊기지 않았다(이전 추정 오류 — `.json.gz`만 보고 `.json` 미압축 최신 파일을 놓침). 6/8~6/12 1MB대 일별 로그 존재. 이 로그(구독 라이프사이클 + `missing_reason`, raw tick 미기록)로 a1 직접 확인: **6/9 구독 30종목 중 18종목, 6/12 34종목 중 21종목이 종일 `subscribed_no_tick`**(≈55%). polling 매수 종목이 두 그룹으로 갈림 — **a1(종일 no-tick)**: 160980(6/9)/200470(6/9·6/12)/403870(6/9·6/12), **tick 정상 수신**: 023530(6/9)/093370(6/9·6/12)/033780(6/10). 시스템 원인: `connection_lost` 12~18/일 + `price_data_gap`가 183→253→322→…→592s로 단조 증가(180s 임계마다 `force_reconnect` 재발) = price 채널이 force-reconnect로도 치유 안 됨. ACK 수정(#509)으로 미해소. → **a1은 WS 피드 건강성 문제로 확정**(shadow뿐 아니라 라이브 실시간 데이터 전반 영향).
  - **잔여 미스터리(tick 수신 그룹)**: 093370/023530/033780은 tick이 들어왔는데도 shadow 0건 + `shadow_eval_stats` 비어있음 → ticks 도착했는데 `evaluate_single` 미실행(또는 6/10 계측이 배포 프로세스에 미반영). 용의자: 공유 `_bought_today` 게이트(polling 선매수) 또는 router 미디스패치. PR #533 `tick_ingest.dispatched` 카운터가 다음 장에서 국소화.
  - 진단 계측 추가(2026-06-13, PR #533): a1/a2 및 dispatched 갭 국소화용 종목별 tick-ingest 카운터 도입. `PriceStreamService` 에 `received`(유효 payload 진입)/`quality_reject`(품질게이트 탈락)/`dispatched`(router 전달) 누적 카운터 + `tick_ingest_stats_snapshot(codes)`. `StrategyScheduler` 에 optional `price_stream_service` DI 추가 → `subscriptions_refreshed` details 에 후보별 `tick_ingest` 스냅샷 첨부(미주입 시 no-op). `strategy_factory` 배선. 테스트: `test_price_stream_service_tick_ingest_stats.py` 5종 + `test_strategy_scheduler_event_shadow.py` 2종. 단위 5942 / 통합 240 passed. (관측 전용, 실주문 경로 무변경)
  - **디스패치 갭 해소(2026-06-16, #551)**: `tick 수신 그룹`(093370/023530/033780) 미스터리 근본 원인 확정·수정. `StreamingService` 가 `realtime_price` 핸들러(`PriceStreamService.on_price_tick`)를 스레드풀 executor 로 오프로드했는데, `on_price_tick` 은 내부에서 `event_router.on_price_tick`(async) 을 running loop 로 디스패치해야 하므로 executor 스레드에선 루프 부재로 디스패치가 끊겼다 → tick 이 `received` 로 잡혀도 `dispatched`=0 → `evaluate_single` 미실행(#533 카운터로 국소화한 그대로). 수정: `on_price_tick` 을 `_event_loop_handlers` 로 등록해 executor 오프로드 대신 현재 루프에서 직접 실행. 테스트: `test_streaming_service.py::test_price_stream_handler_dispatches_event_router_from_streaming_service`. 이로써 **tick 이 도착하는 종목은 shadow 평가가 실행**된다(parity 수집 가능). 남은 차단은 아래 무틱 그룹 단독.
  - **다음 우선순위(잔여 단독 블로커)**: WebSocket price 채널 force-reconnect 미치유 루프(price_data_gap 단조 증가) 근본 원인 — 구독 종목 ≈55%가 종일 무틱이라 shadow parity 수집 자체가 불가. #551 디스패치 수정으로도 무틱 그룹은 안 풀린다(틱이 아예 안 들어옴). 라이브 실시간 데이터 품질 문제이므로 별도 우선 과제로 격상. (06-13~16 커밋에 WS 피드 건강성/force-reconnect 수정 없음 — 미착수 상태 유지)
...
- 적용 완료(2026-06-13): point-in-time universe 1차 독립 경로 추가. `services/point_in_time_universe_service.py`가 현재 상장 목록 + FDR 상폐 목록을 표준 record로 정규화하고, `as_of_date` 기준으로 상장일 이후·상폐일 직전까지만 snapshot에 포함한다(상폐일 당일 제외, 미래 IPO lookahead 제외, 중복 symbol은 delisted history 우선). `scripts/build_point_in_time_universe.py`는 as-of snapshot JSON/CSV/Markdown과 survivorship gap(delisted-only/current-only) 리포트를 출력한다.
- 적용 완료(2026-06-13): 상폐 OHLCV 백필 독립 스크립트 추가. `scripts/backfill_delisted_ohlcv.py`가 FDR `KRX-DELISTING` OHLCV를 종목별 CSV + `index.json`으로 저장한다. 아직 기존 OHLCV repository/백테스트 데이터 provider에는 연결하지 않고, 수동 백필 산출물 생성 단계로 한정했다.
- 적용 완료(2026-06-13): 상폐 포함 전후 성과 영향 1차 리포트 추가. `scripts/report_survivorship_bias_impact.py`가 PIT universe JSON과 `scripts.run_backtest --output json` 결과를 입력으로 받아 delisted-only/current-only SOLD trade net PnL을 분리 집계한다. 실제 성과 격차 확정은 백테스트 runner가 PIT universe + 상폐 OHLCV를 사용하도록 연결한 뒤 재실행해야 한다.
- 적용 완료(2026-06-13, 연결층 1단계 — universe 질의): `services/point_in_time_universe_provider.py` 추가. 정규화된 PIT 레코드(current+delisted)를 받아 백테스트 임의 날짜에 대해 `listed_codes_as_of(date)`/`delisted_codes_as_of(date)`로 "그 날 상장돼 있던 종목(상폐 직전까지 포함)" 집합을 돌려준다. `from_snapshot_payload`(build 스크립트 JSON `records` 키)/`from_records_dicts`/dataclass 입력, symbol zfill·YYYYMMDD↔YYYY-MM-DD 정규화, dedupe는 상폐 이력 우선. 순수 로직이라 FDR 없이 synthetic 레코드로 검증. 주의: 기간 백테스트엔 단일 as-of 스냅샷이 아니라 기간 중 상폐 종목까지 담은 전체 레코드를 넘겨야 한다(다음 단계에서 build 스크립트 full-records export 필요). 테스트: `test_point_in_time_universe_provider.py` 8종. 단위 5945 / 통합 240 passed. (신규 모듈, production 미import — 회귀 위험 없음)
- 적용 완료(2026-06-13, 연결층 2단계 — 상폐 OHLCV 로더): `services/delisted_ohlcv_store.py` 추가. 백필 산출물(`{symbol}.csv`: date/open/high/low/close/volume/change)을 읽어 `get_recent_daily_ohlcv` 정규화 행 스키마(`date`=YYYYMMDD 오름차순, open/high/low/close=float, volume=int)로 서빙한다. `from_backfill_dir`(디렉터리 glob `*.csv`)/직접 rows 생성자, `get_daily_rows(code, limit, end_date)`(end_date YYYYMMDD/YYYY-MM-DD 이하 필터 + 최근 limit), symbol zfill·date compact 정규화·미상장 코드 빈 리스트·없는 디렉터리 안전 처리. 순수 소스라 synthetic CSV로 검증. 테스트: `test_delisted_ohlcv_store.py` 11종. 단위 5956 / 통합 240 passed. (신규 모듈, production 미import — 회귀 위험 없음)
- 적용 완료(2026-06-13, 연결층 3a — 상폐 OHLCV fallback 배선): `StockQueryBacktestReplayService`에 opt-in `delisted_ohlcv_store` 주입. `get_recent_daily_ohlcv`가 primary sqs 응답에 행이 없으면(`_response_has_rows`=False) `DelistedOhlcvStore.get_daily_rows(limit,end_date)`로 fallback해 `ResCommonResponse(SUCCESS, data=rows)` 반환. store 미주입 시 기존 위임 동작 무변경(default off). 테스트: `test_backtest_replay_adapter.py` 5종(fallback/primary 우선·store 미조회/미주입 무변경/store도 비면 primary/limit·end_date 적용). 단위 5961 / 통합 240 passed. 한계: get_current_price/intraday(체결 시뮬)는 상폐 분봉이 없어 미지원 — 상폐 종목은 일봉 기반 후보평가·MtM 경로에서만 다뤄짐.
- 적용 완료(2026-06-13, 연결층 3b — universe PIT 증강 wrapper): `services/point_in_time_universe_wrapper.py` 추가. 라이브 universe **무변경**(백테스트 전용 wrapper). `PointInTimeAugmentedUniverse(base, pit_provider, sqs, clock, item_factory, min_avg_trading_value_5d)`가 base `get_watchlist()` 결과에 `delisted_codes_as_of(as_of)` 상폐 종목을 추가한다. 사용자 결정(거래대금 게이트만): 상폐 종목은 시총 데이터 부재로 시총 필터 면제, 백필 OHLCV(3a fallback) 기반 5일 평균 거래대금(close×volume) 게이트만 적용. end_date=as_of로 lookahead 방지, base 중복/제외 코드 스킵, item 타입은 universe별로 다르므로 `item_factory` 주입, 나머지 메서드(`is_market_timing_ok` 등)는 `__getattr__` 위임, `exclude_code_for_today`는 self+base 양쪽. 테스트: `test_point_in_time_universe_wrapper.py` 7종. 단위 5968 / 통합 240 passed. (신규 모듈, production 미import)
- 적용 완료(2026-06-16, 연결층 4단계 — run_backtest 배선): (1) `build_point_in_time_universe.py`에 `--full-records` 모드 추가 — as-of 필터 없이 current+delisted 전체 정규화 레코드를 `records`로 출력해, provider가 기간 중 임의 날짜 as-of 필터를 수행하도록 한다(단일 as-of 스냅샷은 기간 중간 상폐 종목을 놓침). `build_full_records_payload()` 신설, `--as-of-date`는 `--full-records`와 택일(둘 다 없으면 parser.error), full 모드에선 survivorship_gap/markdown 생략. (2) `run_backtest.py`에 opt-in 플래그 `--pit-universe <full-records.json>`/`--delisted-ohlcv-dir <dir>`/`--pit-min-trading-value` 추가 → `_load_pit_provider`/`_load_delisted_ohlcv_store` 로 로드, `StockQueryBacktestReplayService(delisted_ohlcv_store=...)` 주입, baseline(non-ablation) universe를 `PointInTimeAugmentedUniverse`로 래핑(`_wrap_pit_universe`). 핵심 발견: 활성 전략 universe(Oneil/GenericLiquidity/VboVolatility)가 **모두 `OSBWatchlistItem`을 반환**하므로 전략별 분기 없이 단일 `_make_osb_pit_item_factory`(상폐=`market_cap=0`)로 충분 — VBO `_passes_validity_filter`가 `market_cap>0`일 때만 시총 게이트를 적용해 상폐 종목은 시총 면제·거래대금 게이트만 적용(설계 의도와 일치 확인). ablation/parameter-stability variant와는 합성하지 않음(baseline 전용). 테스트: `test_build_point_in_time_universe.py`(full-records 4종 추가), `test_run_backtest_pit_universe.py`(9종 신설 — 플래그 파싱/provider·store 로드/factory 시총면제/VBO 필터 통과/wrapper 상폐 합류). 단위 6193 / 통합 243 passed.
- 적용 완료(2026-06-16, 실제 격차 산출 1차): FDR로 데이터 생성 + 정량화. `data/survivorship/full_records.json`(current 2,875 + delisted 1,964 = 4,839), `data/survivorship/delisted_ohlcv/`(2025-01~2026-06 비-SPAC 상폐 57종 일봉), 결과 리포트 `data/survivorship/survivorship_exposure_report.md`. **유니버스 노출(실수치)**: 18개월간 유동성 10억+ 상폐 22종 중 부실/감사의견성 ≈15종이 고점 대비 -40~-100% 붕괴(셀리버리 -99.8%/이트론 -98.2%/이아이디 -97.6%/파멥신 -96.1%...) — 전부 survivor-only 유니버스(2026-02 스냅샷)에 0% 표본. **PnL 격차(일봉 VBO 근사, `OverseasDailyVBOBacktest` 재사용)**: 생존 KOSDAQ 69종 vs 상폐 유동성 종목 동일 게이트 비교 → GAP ≈ 0(win -0.2%p, avg_net -0.04%p). **단 이건 측정 아티팩트**: (1) 순진 일봉 근사가 전 종목 균일 손실(생존 avg -1.56%)이라 디플레이트할 baseline 없음, (2) 같은날 EOD 청산+-3% stop이 상폐 종말(거래정지·정리매매 -90%) 봉을 원천 차단. **결론: 생존편향 PnL 비용은 같은날 청산 VBO엔 작고, 붕괴를 들고 가는 멀티데이/오버나잇 전략(Oneil 계열)에 집중**.
- 적용 완료(2026-06-16, R-1 PnL 완결 — 멀티데이): `strategies/multiday_daily_breakout_backtest.py`(신규, 20일 돌파 진입 + 손절/트레일링/시간청산 + gap-through stop + 종말 강제청산) + 단위 6종. 생존 KOSDAQ 104종 vs 상폐 유동성 종목 멀티데이 비교. **결론: 생존편향 PnL 비용은 청산 규율의 함수.** 실제 전략 파라미터(손절-5%/트레일8%)에선 GAP ≈ -0.15%p/거래(무시 가능, 상폐 거래 최악 -19.4%) — 손절이 -90% 정리매매 *전에* 청산하기 때문. 미보호(손절없음·상폐일까지 보유)에서만 avg -59.8%/거래·11건 -99% 붕괴(단 합병성 코오롱/HD현대 일부 과대). **함의: "오염된 표본 위 게이트 통과" 우려는 의무 손절로 상당 부분 완화 — 손절 규율 자체가 생존편향 방어선.** 잔존 리스크 = halt 후 극단 갭다운(gap-through, R-4 연결). 상세: `data/survivorship/survivorship_exposure_report.md` §4.
- R-1 종합 결론: 노출(섹션)은 크나 이 시스템의 의무 손절이 PnL 생존편향을 작게 만든다. `run_backtest --pit-universe`(#552) intraday 경로는 상폐 분봉 부재로 PnL엔 부적합(노출 합류만). `StrategyLogReportService` 자동 통합은 운영 가치 낮아 보류(결론이 "손절 유지"로 수렴).
...
- **보류** — EventShadowManager 등 god class 분리: 테스트 ~26곳이 scheduler 내부 표면을 직접 잠그고(특히 `scheduler._event_router = ...`/`._event_shadow_journal = ...` **생성 후 재할당** 패턴) 호환 유지에 property setter 등 shim 7~8개가 필요해 순복잡도가 증가한다. 또한 shadow 코드는 P2 2-4 parity 결과에 따라 실주문 승격/폐기가 갈리므로 지금 분리하면 재작업 위험. parity 판정(PR-3 진입 여부) 후 재평가.
  - 재검토 (2026-06-12): 분리 대상 shadow 블록은 L1164~1455 + stop_strategy 해제 블록 + 추적 dict 2개 ≈ 330줄. 재개 조건을 PR-3 판정 결과별로 구체화 — (a) **Go(실주문 승격)**: event-driven 주문 배선 *전에* 분리 선행 권장(경계 확보 후 배선이 안전, shim 중 구독 dict 2개는 자가 테스트만 사용이라 테스트 수정으로 제거 가능), (b) **No-Go(폐기)**: 분리 불필요 — shadow 코드 삭제로 종결, (c) **관찰 연장**: 보류 유지. SignalExecutor(`_execute_signal_inner` ~250줄) 분리는 의존성 최다(이력/알림/롤백/sizing/gate/구독)·동작 안정 상태라 shadow 판정과 무관하게 비권장 유지.
- **보류** — LiveStrategy 인터페이스 승격(`_position_state`/`_save_state`/`_bought_today` 정식 메서드화), 동기 DB/CSV 호출 to_thread 전환: 전략 7개 + 호출 경로 전반을 건드려 단독 PR 필요. 인터페이스 승격은 P3 3-4 lifecycle contract 재설계와 함께 다루는 것이 외과적.
  - 재검토 (2026-06-12): 두 우려 모두 당초 평가보다 **범위가 작다**. ① state 저장 블로킹은 이미 해소 — 전략 `_save_state()`는 이벤트 루프 안이면 `StrategyStateIO.schedule_save()` 백그라운드 atomic write 로 위임한다(P0 0-11). 남은 실제 블로킹 후보는 `VirtualTradeRepository.get_holds_by_strategy`의 호출당 `pd.read_sql_query`(DataFrame 생성) 하나 — `_run_strategy` 사이클당 2회+ 및 get_status 웹 폴링×전략 수로 호출된다. to_thread 일괄 전환 대신 **측정 → 해당 경로만 개선**(단기 캐시 또는 pandas 제거)이 외과적. ② 인터페이스 승격은 LiveStrategy 에 **getattr 위임 기본 구현**(position_state/persist_state/discard_bought_today/exclude_code_for_today/strategy_logger)을 추가하면 전략 7개 무변경으로 가능 — scheduler 호출부 7곳만 교체하는 소형 PR 로 P3 3-4 와 분리 가능. 부수 발견: LiveStrategy 의 `async save_state()` hook(L101)은 어떤 전략도 override 하지 않는 죽은 표면(`load_state`는 구현됨) — 승격 시 정리 대상.
  - [x] 측정·적용 (2026-06-12, 브랜치 `perf/get-holds-latency`): 측정 결과 pandas 경로는 **행 수와 무관하게 호출당 ~9ms 고정**(200행/2,000행/10,000행 모두 median 8.7~9.3ms — pandas 오버헤드 지배), 동일 쿼리 raw cursor 는 0.06~0.48ms. get_status 1회(7전략)당 이벤트 루프 ~60ms 블로킹이 확인되어 `get_holds_by_strategy`만 raw sqlite 경로로 전환 — 전환 후 0.07~0.49ms (約 20~130배). 변환 계약(NULL→None, code 문자열 강제, market_regime JSON→dict, 깨진 JSON→None)은 전환 *전* 특성화 테스트 2종으로 잠근 뒤 동일 통과 확인. `get_holds`/`get_all_trades` 등 비핫패스는 pandas `_to_json_records` 유지(외과적 범위 한정). 단위 5900 / 통합 240 passed.
  - [x] 인터페이스 승격 적용 (2026-06-12, 브랜치 `refactor/live-strategy-interface`): LiveStrategy 에 getattr 위임 기본 구현 5종(`position_state`/`persist_state`/`discard_bought_today`/`exclude_code_for_today`/`strategy_logger`) 추가 — 전략 7개 무변경, scheduler 호출부 6곳 교체(metrics logger×2, bought_today 롤백, universe 당일 제외, position_state/persist 헬퍼). 비 LiveStrategy mock 전략 방어용 getattr/isinstance 가드는 scheduler 에 유지. 인터페이스 테스트 3종 추가. 단위 5901 / 통합 240 passed. 죽은 `async save_state()` hook 은 호출처 0건 확인했으나 인터페이스 표면 제거라 건드리지 않고 유지(언급만).
...
2. **운영 관찰 진행 중**
   - **(선결 블로커) WebSocket price 피드 무틱 ≈55% 근본 원인 해소** (P2 2-4 다음 우선순위) — force-reconnect 미치유 루프(price_data_gap 단조 증가) 수정 전까지 아래 shadow 수집은 사실상 불가. 디스패치 갭(#551)은 해소됐으나 무틱 그룹은 별개 문제. 라이브 실시간 데이터 품질이라 최우선.
   - VBO shadow 5거래일 jsonl 수집 → `scripts/analyze_event_shadow_parity.py` 로 parity 리포트 생성 → PR-3 진입 판정 (P2 2-4 PR-2.5)
     - 디스패치 갭(#551, 2026-06-16) 수정 후 tick 수신 종목은 평가 실행. 무틱 블로커 해소 후 장중 운영일부터 수집일 카운트 재시작.
