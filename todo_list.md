...
## P1. 주문 직전 안전장치와 체결 상태 일관성

실전 투입 전 가장 먼저 처리해야 하는 영역입니다. 목표는 “주문 접수”와 “체결 완료”를 명확히 분리하고, 모든 주문이 공통 Risk Gate를 통과하게 만드는 것입니다.

### 1-1. 실전 주문 기본값과 체결 확정 방식 수정

- [ ] 실전 모드에서는 `finalize_immediately` 기본값을 제거하거나 `False`로 고정한다.
- [ ] 주문 접수 상태와 체결 완료 상태를 명확히 분리한다.
- [ ] 시장가 주문도 주문 접수 즉시 보유 반영하지 않고 체결 확인 후 확정한다.
- [ ] 부분체결, 미체결, 취소, 거부 상태를 주문 FSM에 반영한다.
- [ ] stuck order 감지 결과가 주문 상태 전이에 반영되는지 확인한다.
...
- paper/real 분기별 주문 상태 테스트
- 부분체결 후 잔여 미체결 테스트
- 주문 접수 성공, 체결 미확정 상태 테스트

### 1-2. 주문, 잔고, 미체결 reconcile 추가

- [ ] 미체결 주문 조회 wrapper를 추가한다.
- [ ] 체결 내역 조회 wrapper를 추가한다.
- [ ] 잔고 조회와 주문 상태를 비교하는 reconcile 메서드를 추가한다.
- [ ] 서비스 재시작 시 `_order_states`, `_order_no_index`를 복원하는 함수를 추가한다.
- [ ] reconcile 결과 불일치가 있으면 신규 주문을 차단하거나 경고 상태로 전환한다.
...
- 재시작 후 미체결 주문 복원 테스트
- 잔고에는 있는데 내부 상태에는 없는 포지션 탐지 테스트
- 내부 active order와 KIS 미체결 조회 불일치 테스트

### 1-3. 주문 실패, 재시도, fallback 정책 정리

- [ ] 주문 실패 사유별 재시도 가능 여부를 분류한다.
- [ ] API 일시 오류와 주문 거부를 구분한다.
- [ ] 부분체결 후 잔량 자동 취소/재주문 정책을 설정화한다.
- [ ] 지정가 실패 후 시장가 fallback 허용 여부와 조건을 분리한다.
- [ ] 재시도 중 중복 주문이 발생하지 않도록 idempotency key 또는 주문 intent ID를 도입한다.
