...
- [~] 전략별 `entry_reason`, `invalidation_price`, `stop_loss_price`, `target_price`, `trailing_rule`, `expected_holding_period_days`, `confidence`, `required_data`, `config_hash`가 journal 분석까지 이어지는지 샘플 리포트로 검증한다.
  - 조사 결과(2026-05-31): 9개 필드 중 **`config_hash`만** journal(`VirtualTradeRepository`)에 컬럼으로 persist되고 `get_standard_journal_records`/리포트까지 이어진다. `invalidation_price`/`stop_loss_price`는 scheduler `_signal_price_policy_kwargs`로 **order 경로**(`OrderExecutionService`)에만 전달되고 journal에는 안 남는다. `entry_reason`/`target_price`/`trailing_rule`/`expected_holding_period_days`/`confidence`/`required_data`는 `TradeSignal`에는 있으나 `log_buy_async` 시그니처/스키마에 없어 **전부 드롭**된다.
  - 남은 작업(별도 결정 필요): 나머지 8개 필드를 journal 분석까지 잇는 것은 trade audit 경로 멀티레이어 변경(`VirtualTradeRepository` SQLite 스키마 마이그레이션 + `log_buy/log_buy_async` 시그니처 + scheduler `_virtual_trade_log_kwargs` + `to_journal_record`/리포트 surfacing)이라 "검증" 범위를 넘는다. 어떤 필드까지 실제로 persist할지(예: 손절/목표/무효화 3개만, 또는 전체) 결정 후 진행.
