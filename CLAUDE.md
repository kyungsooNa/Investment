...
```
- MCP 환경에서는 `pull_request_read` (`method: "get_status"`)를 반복 호출해 필수 체크(`Python CI`)가 끝날 때까지 기다린다. `sleep` 대기 대신 조회를 30~60초 간격으로 반복한다.
...
- CI 실패 원인이 이번 변경 범위 밖(인프라·외부 API·기존 회귀)인 경우
- 사용자가 머지 보류를 지시한 경우
