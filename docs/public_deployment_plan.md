...
Phase 2에서는 설정 기반 사용자 저장소, `viewer`·`operator`·`admin` 역할이 서명된 세션, API·페이지 권한 행렬, 실전 주문의 admin 이중 gate와 권한 감사 로그를 구현했다. 사용자 비활성화나 역할 변경이 감지되면 기존 세션을 폐기한다.
...
역할 분리는 설정 기반 사용자 저장소, 역할을 포함한 세션, 로그인 발급 로직과 중앙 권한 정책으로 구현한다.
...
|---|---:|---:|---:|
| 시세·차트·랭킹·전략 리포트 조회 | 허용 | 허용 | 허용 |
| 잔고·운영 상태·알림·스케줄러 조회 | 차단 | 허용 | 허용 |
| 모의 주문·일반 구독 및 운영 작업 | 차단 | 허용 | 허용 |
| 실전 주문·일괄 매도 | 차단 | 차단 | 허용 |
| 서버 종료·재시작·Kill Switch 변경 | 차단 | 차단 | 허용 |
| 스케줄러 변경·강제 배치·한도 변경 | 차단 | 차단 | 허용 |
...
- 사용자 비활성화 또는 role 변경 시 기존 세션 폐기
- 서버 렌더 페이지도 API와 동일한 역할 검사를 broker 조회 전에 적용
- 기존 `auth.username`·`auth.password_hash` 단일 계정은 마이그레이션 기간에만 `admin`으로 호환

구현 대상:

- `view/web/user_repository.py`
- `view/web/authorization.py`
- `view/web/security.py`
- `view/web/routes/auth.py`
- `view/web/api_common.py`
- 주문·시스템·스케줄러 라우트
