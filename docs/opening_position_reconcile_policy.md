...
`broker_reconciled`는 실제 전략 로직이 붙지 않은 이름표일 뿐이라, 발견 당일 1회 경보 이후로는 아무도 손절/청산을 관리하지 않는다. 이를 그대로 두면 손실이 나도 알림 없이 계속 방치될 수 있다.

`OpeningPositionReconcileService.reconcile_once()`는 매 실행마다 `broker_reconciled` 전략의 `HOLD` 중 등록일로부터 `STALE_BROKER_RECONCILED_DAYS`(기본 7일) 이상 지난 항목을 `stale_broker_reconciled`로 별도 보고한다.

```python
{
    "stale_broker_reconciled": [
        {"code": "006110", "days_held": 54},
    ],
}
```

`OpeningPositionReconcileTask.run_once()`는 이 목록의 종목별로 `reconcile:broker_reconciled_stale:{code}` dedup_key의 OperatorAlert를 report한다. 목록에서 빠지면(매도 또는 다른 전략으로 재지정) 해당 경보를 resolve한다. `mismatch_count`에는 포함되지 않으며 `reconcile:opening` 경보와는 별개로 관리된다.
