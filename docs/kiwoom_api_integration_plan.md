...
키움 REST API 사용 전 아래 항목을 준비한다.

1. 키움증권 계좌 개설
   - 키움 REST API는 키움증권 계좌 보유 고객 대상 서비스다.

2. 키움 REST API 포털 로그인
   - `https://openapi.kiwoom.com`에서 로그인한다.

3. API 사용신청
   - 포털의 `API 사용신청` 메뉴에서 앱을 등록한다.
   - 운영(real)과 모의투자(paper/demo)는 App Key / Secret이 다르므로 각각 발급한다.

4. 실전 App Key 관리
   - 실전투자 키를 사용할 경우 계좌 App Key 관리 화면에서 호출 IP 등록 필요 여부를 확인한다.
   - 모의투자는 IP 등록 요구가 다를 수 있으므로 포털 안내를 기준으로 확인한다.

5. 모의투자 신청
   - 실제 주문 전 모의투자 환경을 먼저 신청하고, 모의투자 App Key / Secret으로 연결 테스트를 진행한다.

6. 로컬 설정 입력
   - 발급받은 값은 `config/config.yaml`의 `kiwoom.paper` / `kiwoom.real`에 입력한다.
   - 키와 계좌번호는 저장소에 커밋하지 않는다.
...
공식 문서에서 확인한 기본 도메인은 운영 `https://api.kiwoom.com`, 모의투자 `https://mockapi.kiwoom.com`이며, 접근토큰 발급 URL은 `/oauth2/token`이다.
