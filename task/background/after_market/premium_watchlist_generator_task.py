...
class PremiumWatchlistGeneratorTask(AfterMarketTask):
    ...
    async def _on_market_closed(self, latest_trading_date: str) -> None:
        ...
        if self._last_generated_date == latest_trading_date:
            if self._ns:
                await self._ns.emit(
                    NotificationCategory.BACKGROUND, "info", "전일기준우량주 생성 스킵",
    ...
    async def _run_generation(self, trading_date: str) -> None:
        ...
        try:
            ...
            if self._ns:
                await self._ns.emit(
                    NotificationCategory.BACKGROUND, "info", "전일기준우량주 생성 완료",
            ...
            if self._ns:
                await self._ns.emit(NotificationCategory.BACKGROUND, "error", "전일기준우량주 생성 실패", str(e))
        finally:
