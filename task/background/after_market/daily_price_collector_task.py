...
class DailyPriceCollectorTask(AfterMarketTask):
    ...
    async def _collect_all_prices(self, force: bool = False) -> None:
        ...
        if not force and self._last_collected_date == target_date:
            ...
            if self._ns:
                await self._ns.emit(NotificationCategory.BACKGROUND, "info", "현재가 수집 스킵", f"{target_date} 이미 수집 완료된 상태입니다.")
            self._is_collecting = False
            return
        ...
        try:
            ...
            if self._ns:
                await self._ns.emit(
                    NotificationCategory.BACKGROUND, "info", "전체 종목 현재가 수집 완료",
            ...
            if self._ns:
                await self._ns.emit(NotificationCategory.BACKGROUND, "error", "현재가 수집 실패", str(e))
        finally:
            self._is_collecting = False
