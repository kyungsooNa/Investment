...
async def test_get_price_summary(mock_quotations):
    # 1. Mock 설정: ResCommonResponse 형식으로 반환하도록 수정
    mock_quotations.get_current_price = AsyncMock(return_value=ResCommonResponse(
        rt_cd=ErrorCode.SUCCESS.value,  # 성공 코드 명시
        msg1="성공",
        data={  # 원래 output이었던 데이터를 'data' 필드에 넣습니다.
            "stck_oprc": "10000",
            "stck_prpr": "11000",
            "prdy_ctrt": "10.0"  # prdy_ctrt도 추가해야 합니다. get_price_summary가 이 값을 사용합니다.
        }
    ))
...
async def test_get_price_summary_invalid_response(mock_quotations):
    ...
    # mock_quotations.logger.warning.assert_called_once() -> get_current_price 내부 로깅 확인은 해당 테스트에서 수행


@pytest.mark.asyncio
async def test_get_top_market_cap_stocks_success(mock_quotations):
    # get_stock_info_by_code는 get_top_market_cap_stocks_code에서 직접 호출되지 않습니다.
    # mock_quotations.get_stock_info_by_code = AsyncMock(return_value={}) # 이 라인은 불필요
...
async def test_get_top_market_cap_stocks_failure(mock_quotations):
    ...
    assert result_common.rt_cd== "1"  # API에서 온 실패 코드
...
async def test_get_stock_info_by_code_failure(mock_quotations):
    mock_quotations.call_api = AsyncMock(return_value={"rt_cd": "1", "msg1": "API 실패 메시지", "output": None})
    result_common = await mock_quotations.get_stock_info_by_code("005930")
...
async def test_get_stock_info_by_code_parsing_error(mock_quotations):
    ...
    mock_quotations.call_api = AsyncMock(return_value={"rt_cd": "0", "output": {"some_unknown_key": "value"}})
...
async def test_get_top_market_cap_stocks_failure_rt_cd_not_zero(mock_quotations):
    ...
    assert result_common.rt_cd== "1"
    assert result_common.msg1 == "API 호출 실패"
...
async def test_get_top_market_cap_stocks_count_validation(mock_quotations):
    ...
    mock_quotations.logger.warning.assert_called_once_with("요청된 count가 0 이하입니다. count=0")  # 정확한 메시지 확인
    ...
    mock_quotations.call_api.assert_not_called()
    mock_quotations.logger.warning.assert_called_once_with("요청된 count가 0 이하입니다. count=-5")
    ...
    mock_api_response_large = {
        "rt_cd": "0",
        "output": [{"iscd": f"{i:06d}", "mksc_shrn_iscd": f"{i:06d}", "stck_avls": f"{1000000000 + i}"} for i in
                   range(40)]
    }
    ...
    mock_quotations.call_api.assert_called_once()
    mock_quotations.logger.warning.assert_called_once_with("요청 수 50는 최대 허용값 30을 초과하므로 30개로 제한됩니다.")
...
async def test_get_filtered_stocks_by_momentum_price_summary_failure(mock_quotations):
    ...
    mock_quotations.logger.warning.assert_called_once()  # get_price_summary 실패 경고 로그
...
async def test_get_filtered_stocks_by_momentum_prev_day_info_failure(mock_quotations):
    ...
    assert results_common.data == []  # 결과는 비어있음
    mock_quotations.logger.warning.assert_called_once()  # get_previous_day_info 실패 경고 로그
...
async def test_get_top_market_cap_stocks_item_missing_keys(mock_quotations):
    ...
    mock_api_response = {
        ...
        "output": [
            ...
            {"mksc_shrn_iscd": "000660"},  # stck_avls 없음 -> 건너뜀
            {"stck_avls": "100,000,000,000"},  # iscd/mksc_shrn_iscd 없음 -> 건너뜀
            {"iscd": "000770", "stck_avls": "INVALID"}  # 유효하지 않은 시총 -> market_cap 0으로 처리
        ]
    }
...
async def test_inquire_daily_itemchartprice_call_api_none(mocker, api_instance):
    ...
    assert result_common.rt_cd== ErrorCode.API_ERROR.value  # Enum 값 사용
