...
프로그램매매 워치독/복원/재연결 및 태스크 통합 관리 검증.
"""
import asyncio
import time
import pytest
...
@pytest.mark.asyncio
async def test_paper_trading_still_works(bg_service, mock_deps):
...
@pytest.mark.asyncio
async def test_on_demand_trigger_when_cache_empty(bg_service, mock_deps):
...
@pytest.mark.asyncio
async def test_no_trigger_when_already_refreshing(bg_service):
...
async def test_refresh_investor_ranking_basic(bg_service, mock_deps):
    ...
    buy_resp = await bg_service.get_foreign_net_buy_ranking()
    ...
    sell_resp = await bg_service.get_foreign_net_sell_ranking()
    ...
    inst_buy = await bg_service.get_inst_net_buy_ranking()
    ...
    # 기관 순매도 상위: NAVER(-100)
    inst_sell = await bg_service.get_inst_net_sell_ranking()
    assert inst_sell.data[0]["hts_kor_isnm"] == "NAVER"

    # 개인 순매수 상위: SK(400) > NAVER(50) > 삼성(-300)
    prsn_buy = await bg_service.get_prsn_net_buy_ranking()
...
@pytest.mark.asyncio
async def test_get_foreign_ranking_with_limit(bg_service):
    ...
    resp = await bg_service.get_foreign_net_buy_ranking(limit=5)
...
async def test_refresh_basic_ranking(mock_deps):
    ...
    bg.mdm = market_date_manager # Ensure mdm is set for this test
...
@pytest.mark.asyncio
async def test_check_and_trigger_refresh_no_loop(bg_service):
    ...
    with patch("asyncio.get_running_loop", side_effect=RuntimeError("No running loop")):
        with patch("asyncio.create_task") as mock_create_task:
            await bg_service._check_and_trigger_refresh()
...
async def test_refresh_program_ranking_basic(bg_service, mock_deps):
    ...
    buy_resp = await bg_service.get_program_net_buy_ranking()
    assert buy_resp.rt_cd == ErrorCode.SUCCESS.value
    assert len(buy_resp.data) == 3
    assert buy_resp.data[0]["hts_kor_isnm"] == "삼성전자"
    ...
    # 프로그램 순매도 상위: SK(-200만)
    sell_resp = await bg_service.get_program_net_sell_ranking()
    assert sell_resp.rt_cd == ErrorCode.SUCCESS.value
    assert sell_resp.data[0]["hts_kor_isnm"] == "SK하이닉스"
    assert sell_resp.data[0]["data_rank"] == "1"
...
@pytest.mark.asyncio
async def test_get_trading_value_ranking_populated(bg_service):
...
async def test_refresh_investor_ranking_corrects_acml_tr_pbmn(bg_service, mock_deps):
    ...
    # 거래대금 랭킹 확인
    tv_resp = await bg_service.get_trading_value_ranking()
...
@pytest.mark.asyncio
async def test_scheduler_skips_if_already_updated_for_trading_date(bg_service, mock_deps):
    """장 마감 후 해당 거래일에 대해 이미 갱신 완료되었으면 재갱신하지 않는다.

    시나리오: 3/15 16:00에 갱신 완료, 같은 날 20:00에 스케줄러 재실행.
    최근 거래일(20260315) = 갱신 이력(20260315) → 스킵.
    """
    import asyncio
    _, _, _, _, _, market_date_manager = mock_deps
...
@pytest.mark.asyncio
async def test_scheduler_no_double_refresh_overnight(bg_service, mock_deps):
    """자정~09시 사이, 전날 장 마감 후 갱신 이력이 있으면 재갱신하지 않는다.

    시나리오: 3/15 16:00에 갱신 완료 → 12시간 딥슬립 → 3/16 04:00에 깨어남.
    최근 거래일은 여전히 20260315이므로, 이미 갱신된 20260315와 일치 → 스킵.
...
# ══════════════════════════════════════════════════════════
# 태스크 통합 관리 (start_all / shutdown) 테스트
# ══════════════════════════════════════════════════════════


@pytest.fixture
def bg_service_full(mock_deps):
    """프로그램매매 관련 의존성까지 포함된 BackgroundService fixture."""
    broker, mapper, env, logger, time_manager, market_date_manager = mock_deps
    trading_service = MagicMock()
...
@pytest.mark.asyncio
async def test_start_all_creates_tasks(bg_service_full):
    """start_all이 백그라운드 태스크를 생성하고 추적하는지 검증."""
    svc = bg_service_full
    svc._realtime_data_manager.get_subscribed_codes.return_value = ["005930"]

    # 무한 루프 코루틴들이 실행되지 않도록 모킹
    svc._restore_program_trading = AsyncMock()
    svc._program_trading_watchdog = AsyncMock()
    svc.refresh_investor_ranking = AsyncMock()
    svc.start_after_market_scheduler = AsyncMock()
    ...
    with patch("services.background_service.asyncio.create_task", side_effect=coro_collector):
        svc.start_all(realtime_callback=MagicMock())
    ...
    # RDM 백그라운드 태스크 시작
    svc._realtime_data_manager.start_background_tasks.assert_called_once()
    # 4개 태스크: restore + watchdog + ranking + scheduler
    assert len(created_coros) == 4
    svc._restore_program_trading.assert_awaited_once_with(["005930"])
    svc._program_trading_watchdog.assert_awaited_once()
    svc.refresh_investor_ranking.assert_awaited_once()
    svc.start_after_market_scheduler.assert_awaited_once()


@pytest.mark.asyncio
async def test_start_all_no_restore_when_no_saved_codes(bg_service_full):
    """저장된 구독 종목이 없으면 restore 태스크를 생성하지 않는다."""
    svc = bg_service_full
    svc._realtime_data_manager.get_subscribed_codes.return_value = []
    ...
    # 3개 태스크: watchdog + ranking + scheduler (restore 없음)
    assert len(created_coros) == 3


@pytest.mark.asyncio
async def test_shutdown_cancels_all_tasks(bg_service_full):
    """shutdown이 모든 추적 중인 태스크를 취소하는지 검증."""
    svc = bg_service_full
    ...
    with patch("services.background_service.asyncio.gather", new_callable=AsyncMock):
        await svc.shutdown()

    mock_task1.cancel.assert_called_once()
    mock_task2.cancel.assert_not_called()  # 이미 완료된 태스크는 취소하지 않음
    assert len(svc._tasks) == 0
    svc._realtime_data_manager.shutdown.assert_awaited_once()


# ══════════════════════════════════════════════════════════
# 프로그램매매 복원 / 워치독 / 재연결 테스트
# ══════════════════════════════════════════════════════════


@pytest.mark.asyncio
async def test_restore_program_trading_success(bg_service_full):
...
@pytest.mark.asyncio
async def test_restore_program_trading_partial_failure(bg_service_full):
...
@pytest.mark.asyncio
async def test_program_trading_watchdog_market_closed(bg_service_full):
    ...
    with patch("services.background_service.asyncio.sleep", side_effect=sleep_side_effect):
        try:
...
@pytest.mark.asyncio
async def test_program_trading_watchdog_data_gap(bg_service_full):
...
@pytest.mark.asyncio
async def test_force_reconnect_program_trading(bg_service_full):
