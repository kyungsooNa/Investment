...
from strategies.oneil.common_types import OSBWatchlistItem
...
async def test_analyze_candidate_success(mock_deps):
    ...
    # 최근 50일 데이터 생성
    # [수정] 거래대금 필터(100억 이상) 통과를 위해 거래량 상향 조정 (1000원 * 1500만주 = 150억)
    ...
    ts.get_current_stock_price.return_value = ResCommonResponse(
        rt_cd="0", msg1="success", data={"output": {"w52_hgpr": "1200", "stck_llam": "100000000000"}}
    )
    
    # 3. BB Mock (스퀴즈 데이터 계산용)
    indicator.get_bollinger_bands.return_value = ResCommonResponse(
        rt_cd="0", msg1="success", data=[MagicMock(upper=110, lower=90) for _ in range(30)]
    )
    
    # 4. RS Mock
    indicator.get_relative_strength.return_value = ResCommonResponse(
        rt_cd="0", msg1="success", data=MagicMock(return_pct=10.0)
    )
...
async def test_generate_pool_a(mock_deps):
    ...
    async def mock_get_price(code):
        if code == "000001":
            return ResCommonResponse(rt_cd="0", msg1="success", data={"output": {"stck_llam": "500000000000", "acml_tr_pbmn": "20000000000"}})
        return ResCommonResponse(rt_cd="0", msg1="success", data={"output": {"stck_llam": "10000000", "acml_tr_pbmn": "100"}})
...
async def test_get_watchlist_refresh_logic(mock_deps):
    ...
    with patch.object(service, '_load_pool_a', return_value=[]), \
        ...
        # [수정] 날짜가 이미 설정된 상태로 시작하여 '초기화' 분기가 아닌 '갱신' 분기를 타도록 유도
        service._watchlist_date = "20250101"

        # 1. 첫 호출: 워치리스트 빌드 수행 (10분 경과 -> 갱신 트리거)
