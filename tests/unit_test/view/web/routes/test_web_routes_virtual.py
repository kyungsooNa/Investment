...
async def test_virtual_endpoints(web_client, mock_web_ctx):
    ...
    mock_web_ctx.virtual_trade_service.save_daily_snapshot.return_value = None
...
async def test_get_virtual_summary_no_manager(web_client, mock_web_ctx):
    ...
    if hasattr(mock_web_ctx, 'virtual_trade_service'): del mock_web_ctx.virtual_trade_service
    response = web_client.get("/api/virtual/summary")
    assert response.status_code == 200
...
async def test_get_strategy_chart_all_and_failure(web_client, mock_web_ctx):
    ...
    mock_web_ctx.virtual_trade_service.get_strategy_return_history.return_value = [{"date": "2025-01-01", "return_rate": 0}]
...
async def test_get_virtual_history_complex(web_client, mock_web_ctx):
    ...
    web_api._PRICE_CACHE.clear()
    # 1. 매니저 없음
    if hasattr(mock_web_ctx, 'virtual_trade_service'):
        vm = mock_web_ctx.virtual_trade_service
        del mock_web_ctx.virtual_trade_service
    resp = web_client.get("/api/virtual/history")
    assert resp.json()["trades"] == []

    # 매니저 복구
    mock_web_ctx.virtual_trade_service = vm
    ...
    mock_web_ctx.stock_query_service.get_multi_price = mock_multi_price
    mock_web_ctx.virtual_trade_service.fix_sell_price = MagicMock()
...
async def test_get_virtual_history_internal_exceptions(web_client, mock_web_ctx):
    ...
    mock_web_ctx.virtual_trade_service.fix_sell_price.side_effect = Exception("Fix Error")

    # save_daily_snapshot 예외 발생 설정 (Block 4 예외)
    mock_web_ctx.virtual_trade_service.save_daily_snapshot.side_effect = Exception("Snapshot Error")
...
async def test_get_virtual_history_missing_services(web_client, mock_web_ctx):
    ...
    if hasattr(mock_web_ctx, 'stock_code_repository'): del mock_web_ctx.stock_code_repository
...
async def test_get_virtual_history_snapshot_dates_populated(web_client, mock_web_ctx):
    ...
    mock_web_ctx.virtual_trade_service.get_daily_change.return_value = (1.5, "2025-01-02")
    mock_web_ctx.virtual_trade_service.get_weekly_change.return_value = (5.0, "2024-12-25")
    mock_web_ctx.virtual_trade_service._load_data.return_value = {}  # Dummy data
    mock_web_ctx.virtual_trade_service.save_daily_snapshot.return_value = None
...
async def test_get_virtual_history_counts_logic(web_client, mock_web_ctx):
    ...
    mock_web_ctx.virtual_trade_service.get_all_trades.return_value = trades
