...
class TestPerformanceManager(unittest.TestCase):
    ...
    def test_log_timer_disabled(self):
        ...
        pm = PerformanceProfiler(logger=self.mock_logger, enabled=False)
        pm.log_timer("test_func", 100.0)
    ...
    def test_init_with_threshold(self):
        """임계값 설정 초기화 테스트"""
        pm = PerformanceProfiler(threshold=0.5)
    ...
    def test_log_timer_below_threshold(self, mock_time):
        ...
        pm = PerformanceProfiler(logger=self.mock_logger, enabled=True, threshold=0.5)
    ...
    def test_log_timer_override_threshold(self, mock_time):
        ...
        pm = PerformanceProfiler(logger=self.mock_logger, enabled=True, threshold=1.0)
...
class TestPerformanceManagerProfile(unittest.TestCase):
    ...
    @patch('core.performance_profiler.HAS_PYINSTRUMENT', True)
    @patch('core.performance_profiler.pyinstrument', create=True)
    ...
    @patch('core.performance_profiler.HAS_PYINSTRUMENT', False)
    ...
    def test_profile_saves_html(self, mock_pyinstrument):
        ...
        mock_makedirs.assert_called_once_with(PerformanceProfiler.PROFILE_OUTPUT_DIR, exist_ok=True)
