...
@pytest.fixture
def market_clock(mock_logger):
    # 기본값으로 인스턴스 생성, 필요시 테스트에서 재정의
    return MarketClock(logger=mock_logger)
...
@patch("core.market_clock.MarketClock.get_current_kst_time")
...
def test_init_default_values(mock_logger):
    ...
    tm_with_custom_logger = MarketClock(logger=mock_logger)
...
def test_init_custom_values(mock_logger):
    ...
    """
    tm = MarketClock(market_open_time="10:00", market_close_time="16:00", timezone="America/New_York",
                     logger=mock_logger)
...
def test_init_unknown_timezone_error(mock_logger):
    ...
    with patch('pytz.timezone') as mock_pytz_timezone, \
        ...
        tm = MarketClock(timezone="Invalid/TimeZone", logger=mock_logger)
...
def test_get_current_kst_time(market_clock):
    """get_current_kst_time 메서드 커버"""

    with patch('core.market_clock.datetime') as mock_datetime_module:
        ...
        mock_datetime_instance.tzinfo = market_clock.market_timezone

        # now() 함수가 해당 mock 객체를 반환하도록 설정
        mock_datetime_module.now.return_value = mock_datetime_instance

        result = market_clock.get_current_kst_time()

        # 검증
        assert result.hour == 10
        assert result.year == 2025
        assert result.tzinfo == market_clock.market_timezone


def test_is_market_open_during_hours(market_clock):
    """
...
def test_is_market_open_before_open(market_clock):
    """
    is_market_operating_hours 메서드 커버: 평일, 개장 전 시간
    """
    # 평일 개장 전 시간
    weekday_before_open = market_clock.market_timezone.localize(dt.datetime(2025, 6, 27, 8, 30, 0))  # 금요일 8시 30분
    assert not market_clock.is_market_operating_hours(now=weekday_before_open)


def test_is_market_open_after_close(market_clock):
    """
    is_market_operating_hours 메서드 커버: 평일, 폐장 후 시간
    """
    # 평일 폐장 후 시간
    weekday_after_close = market_clock.market_timezone.localize(dt.datetime(2025, 6, 27, 16, 0, 0))  # 금요일 16시
...
def test_sleep_positive_seconds(market_clock, mock_logger):
    ...
    with patch('time.sleep') as mock_time_sleep:
        market_clock.sleep(0.01)  # 아주 작은 값으로 실제 sleep 방지
        mock_time_sleep.assert_called_once_with(0.01)
        mock_logger.info.assert_called_once_with("0.01초 동안 대기합니다 (동기).")


def test_sleep_zero_seconds(market_clock, mock_logger):
    """
    sleep 메서드 커버: seconds <= 0인 경우
    """
    with patch('time.sleep') as mock_time_sleep:
        market_clock.sleep(0)
        mock_time_sleep.assert_not_called()
        mock_logger.info.assert_not_called()


def test_sleep_negative_seconds(market_clock, mock_logger):
    """
    sleep 메서드 커버: seconds <= 0인 경우
    """
    with patch('time.sleep') as mock_time_sleep:
        market_clock.sleep(-1)
        mock_time_sleep.assert_not_called()
        mock_logger.info.assert_not_called()


@pytest.mark.asyncio
async def test_async_sleep_positive_seconds(market_clock, mock_logger):
    """
    async_sleep 메서드 커버: seconds > 0인 경우 (라인 105-107)
    """
    with patch('asyncio.sleep') as mock_asyncio_sleep:
        await market_clock.async_sleep(0.01)  # 아주 작은 값으로 실제 sleep 방지
        mock_asyncio_sleep.assert_called_once_with(0.01)
        mock_logger.info.assert_called_once_with("0.01초 동안 대기합니다 (비동기).")


@pytest.mark.asyncio
async def test_async_sleep_zero_seconds(market_clock, mock_logger):
    """
    async_sleep 메서드 커버: seconds <= 0인 경우
    """
    with patch('asyncio.sleep') as mock_asyncio_sleep:
        await market_clock.async_sleep(0)
        mock_asyncio_sleep.assert_not_called()
        mock_logger.info.assert_not_called()


@pytest.mark.asyncio
async def test_async_sleep_negative_seconds(market_clock, mock_logger):
    """
    async_sleep 메서드 커버: seconds <= 0인 경우
    """
    with patch('asyncio.sleep') as mock_asyncio_sleep:
        await market_clock.async_sleep(-1)
        mock_asyncio_sleep.assert_not_called()
        mock_logger.info.assert_not_called()
...
def test_get_market_open_time(market_clock):
    now = market_clock.get_current_kst_time()
    open_time = market_clock.get_market_open_time()
...
def test_to_yyyymmdd_none_uses_now(market_clock, mocker):
    ...
    fixed = market_clock.market_timezone.localize(dt.datetime(2025, 8, 21, 9, 0, 0))
    mocker.patch.object(market_clock, "get_current_kst_time", return_value=fixed)
    assert market_clock.to_yyyymmdd(None) == "20250821"


def test_to_yyyymmdd_str_passthrough(market_clock):
    # 문자열은 가정상 'YYYYMMDD'로 들어오며 그대로 반환
    assert market_clock.to_yyyymmdd("20250102") == "20250102"


def test_to_yyyymmdd_datetime_and_date(market_clock):
    assert market_clock.to_yyyymmdd(dt.datetime(2025, 7, 1, 12, 34, 56)) == "20250701"
    assert market_clock.to_yyyymmdd(dt.date(2025, 7, 1)) == "20250701"


def test_to_yyyymmdd_callable(market_clock):
    fn = lambda: dt.datetime(2025, 12, 31)
    assert market_clock.to_yyyymmdd(fn) == "20251231"


def test_to_yyyymmdd_numeric_and_other(market_clock):
    # 숫자 등 기타는 str 캐스팅 결과를 반환
    assert market_clock.to_yyyymmdd(20240705) == "20240705"
    assert market_clock.to_yyyymmdd("2024-07-05") == "2024-07-05"  # 포맷 보정은 하지 않음(스펙대로)
...
def test_to_hhmmss_various_inputs(market_clock, raw, expected):
    assert market_clock.to_hhmmss(raw) == expected


def test_to_hhmmss_int_input(market_clock):
    # int도 허용 (자동 str 캐스팅 후 정규화)
    assert market_clock.to_hhmmss(930) == "000930"   # 숫자 '930' → '000930' (뒤 6자리 규칙)


def test_to_hhmmss_none_uses_now_safely(market_clock, mocker):
    """
    ...
    mocker.patch.object(market_clock, "get_current_kst_time", return_value=naive)
    assert market_clock.to_hhmmss(None) == "093100"
...
def test_dec_minute_basic_cases(market_clock, start, minutes, expected):
    assert market_clock.dec_minute(start, minutes) == expected


# --- Sleep Wait Calculation ---

def test_get_sleep_seconds_until_market_close_long_wait(market_clock, mock_logger):
    now = market_clock.market_timezone.localize(datetime(2025, 6, 27, 14, 0, 0))
    close = market_clock.market_timezone.localize(datetime(2025, 6, 27, 15, 30, 0))

    with patch.object(market_clock, 'get_current_kst_time', return_value=now), \
         patch.object(market_clock, 'get_market_close_time', return_value=close):
        seconds = market_clock.get_sleep_seconds_until_market_close()
...
def test_get_sleep_seconds_until_market_close_already_closed(market_clock):
    now = market_clock.market_timezone.localize(datetime(2025, 6, 27, 16, 0, 0))
    close = market_clock.market_timezone.localize(datetime(2025, 6, 27, 15, 30, 0))
