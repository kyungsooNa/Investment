...
class TestTradingServiceBuyOrder(IsolatedAsyncioTestCase):
    ...
    async def test_place_buy_order_failure(self):
        ...
        # 에러 로그 기록 확인
        self.assertTrue(
            any("매수 주문 중 오류 발생" in call_args[0][0] for call_args in self.mock_logger.exception.call_args_list)
        )
    ...
    async def test_place_buy_order_exception_logging(self):
        ...
        self.mock_logger.exception.assert_any_call("Service - 매수 주문 중 오류 발생: 예상치 못한 오류")
