...
def test_extracts_reason_line_after_signal():
    signal, reason, text = extract_signal(
        "신호: 하\n신호 근거: 위험 요인이 3건으로 긍정 요인보다 많음\n한줄 요약: 부정적입니다."
    )

    assert signal == "하"
    assert reason == "위험 요인이 3건으로 긍정 요인보다 많음"
    assert text == "한줄 요약: 부정적입니다."


def test_reason_tolerates_markdown_bold():
    signal, reason, _ = extract_signal("**신호: 중**\n**신호 근거: 호재와 악재가 혼재**\n본문")

    assert signal == "중"
    assert reason == "호재와 악재가 혼재"


def test_reason_without_signal_leaves_text_untouched():
    body = "신호 근거: 근거만 있는 응답\n본문"
    assert extract_signal(body) == (None, None, body)
